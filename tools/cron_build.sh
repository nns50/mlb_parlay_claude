#!/usr/bin/env bash
#
# cron_build.sh — time-aware cron entry point for the MLB parlay routine.
#
# HOW IT WORKS
#   1. Detects the current ET wall-clock time (or accepts a forced build type).
#   2. Selects the matching prompt (09 / 15 / 18 build).
#   3. Invokes `claude --print` (non-interactive) with that prompt.
#   4. The UserPromptSubmit hook fires → injects the session_start.sh digest
#      automatically → Claude runs the full routine and exits.
#
# CRONTAB EXAMPLE (ACTUAL schedule: 3 runs/day at 11:00, 16:00, 18:00 ET — user-confirmed 6/7/26)
#   # MLB parlay routine — 11:00, 16:00, 18:00 ET (adjust TZ as needed)
#   0  11  * * * bash /home/user/mlb_parlay_claude/tools/cron_build.sh 11 >> /tmp/mlb_cron.log 2>&1
#   0  16  * * * bash /home/user/mlb_parlay_claude/tools/cron_build.sh 16 >> /tmp/mlb_cron.log 2>&1
#   0  18  * * * bash /home/user/mlb_parlay_claude/tools/cron_build.sh 18 >> /tmp/mlb_cron.log 2>&1
#
# USAGE
#   tools/cron_build.sh           # auto-detect build from ET time (11→morning, 16→midday, 18→final)
#   tools/cron_build.sh 11        # force the 11:00 morning settle + build (alias: 09)
#   tools/cron_build.sh 16        # force the 16:00 CLV + lineup lock + user-angle build (alias: 15)
#   tools/cron_build.sh 18        # force the 18:00 final check + live-ML settle
#
# REQUIREMENTS
#   • `claude` CLI on PATH (Claude Code — https://claude.ai/code).
#   • ODDS_API_KEY set in the environment (for odds_api.sh calls).
#
set -uo pipefail
cd "$(dirname "$0")/.."

# ── Build type: forced arg OR auto-detect from ET hour ───────────────────────
# ACTUAL SCHEDULE (user-confirmed 6/7/26): the routine fires 3×/day at 11:00, 16:00, 18:00 ET.
#   11:00 ET → "morning"  (settle yesterday + full scan + initial 3-tier build; pre-lineup, props PENDING)
#   16:00 ET → "midday"   (lineup lock for the evening core + CLV + user-angle execution; revise build)
#   18:00 ET → "final"    (late/west-coast lineups + final CLV + live-ML trigger settle)
# Parse args order-independently so `--prompt-only` works WITH or WITHOUT an explicit hour
# (Bug 6/7/26: a bare `--prompt-only` was taken as the build type and hit the error case).
PROMPT_ONLY=0
BUILD=""
for a in "$@"; do
  case "$a" in
    --prompt-only) PROMPT_ONLY=1 ;;
    *) [[ -z "$BUILD" ]] && BUILD="$a" ;;
  esac
done
if [[ -z "$BUILD" ]]; then
  HOUR=$(TZ="America/New_York" date +%H 2>/dev/null || date +%H)
  if   (( 10#$HOUR >= 6  && 10#$HOUR < 14 )); then BUILD="11"
  elif (( 10#$HOUR >= 14 && 10#$HOUR < 17 )); then BUILD="16"
  else BUILD="18"
  fi
fi

# ── Prompts — each is a self-contained instruction for Claude ─────────────────
case "$BUILD" in

11|09|9)
  LABEL="11:00 ET — morning settle + full build (pre-lineup)"
  PROMPT="Run the 11:00 ET daily MLB parlay routine per CLAUDE.md (first of 3 runs: 11:00 / 16:00 / 18:00 ET). This is the morning build — lineups are NOT posted yet, so hitter/K/prop legs are PENDING LINEUP (the 16:00 run locks them). The session_start.sh digest is already in your context (injected by hook). Steps:
1. Self-settle all TBD results from yesterday: run tools/settle.py, apply W/L to results_log.md + fades.md + bankroll.md.
1b. PRIOR-DAY FULL-SLATE REVIEW (MANDATORY per CLAUDE.md, owned by THIS run): retrospect EVERY game from yesterday (session_start's finals list) — one-row-per-game table + 3-6 findings vs active reads (K-Over/Under on notable arms, fade-list teams, rejected legs validated or missed, new hot/cold teams); write it into YESTERDAY's parlays file under '## Full-slate review', update fades.md logs/tallies + promote lessons per the tiered bar. Findings-focused, not a box-score dump. WOULD-HAVE ACCOUNTING IS PRE-REGISTERED: for each benched/governed dimension, the candidate set is FIXED as 'every logged results_log row in that dimension' — settle ALL of them (the 8/6 declined-adjustment control-group row is the template) and NEVER quote a would-have streak computed from a narrative subset (8/7 audit: the 'Angle B 5-0' claim had silently skipped four logged 8/5 candidates; true record was 6-3).
1c. CLV BACKFILL (closes the holes the live runs missed — early slates, dropped runs): run 'python3 tools/clv_backfill.py <yesterday>' (PLAN mode, free). If it shows fillable ML/total/RL rows, re-run with --apply when the plan cost is <=120 credits (pass --max-credits to match); report rows-filled + credits spent in the build section, then the calib/pulse re-run in step 2 picks the new closes up. Prop rows it flags MANUAL stay hand-pulled. A blank CLV cell on a decided leg is a measurement leak into the governor's own shade trigger — this step is how the 'CLV is MANDATORY for EVERY logged leg' doctrine actually gets met.
2. Run tools/calib.py and apply the fresh calibration bands before building.
2b. PULSE (MANDATORY, user-directed 8/1): the session_start digest includes 'python3 tools/pulse.py' — the recent-window exposure governor. APPLY its ACTIONS mechanically: COOL = halve that dimension's adjustments + barred from Tier 1/parlay-anchor; SUSPEND = no new legs in that dimension; MARKET-SHADE = TrueP = market no-vig (adjustments 0) for that dimension; GLOBAL SHRINK = halve every adjustment. These override static registry magnitudes for THIS build (the registry itself only changes at the n>=20-30 evidence bar). Paste the PULSE block into the build section and fill the 'Recent-window pulse applied' gate row with the actions taken.
3. Run tools/mlb_api.sh slate today for the full game list, then 'python3 tools/recheck.py snap today's-date' to snapshot the slate's probables (commit the snapshot with the build — the 16:00/18:00 runs diff against it to catch SP scratches).
4. Do the mandatory slate-wide value scan (ALL games — non-ML markets first per CLAUDE.md).
4b. NRFI/YRFI TRACKER (nrfi_tracker.md — standalone daily tracker, NOT part of the 3-tier build; MANDATORY every run): (a) prior-day reads are AUTO-SETTLED by session_start.sh (tools/nrfi_settle.py --apply stamps W/L from the 1st-inning line score + refreshes the record) — VERIFY the stamps landed; if any final TBD row remains, run 'python3 tools/nrfi_settle.py <yesterday> --apply'. Then update the 'Settled' prose + 'Open' line to match. (b) REPLACE the 'Tonight's reads' section with TODAY's full slate — one NRFI (Under 0.5) / YRFI (Over 0.5) model-lean per game derived from both SPs' 1st-inning quality + both 1-2-3 trios (OBP/power) + park/weather, each with a pre-registered TrueP, and append one ledger row per game with Result TBD (model-only/no-bet unless you pull a real totals_1st_1_innings line that clears the +2pp gate). The dashboard step renders this file, so updating it daily is what keeps the dashboard's NRFI list fresh.
5. Fill SP-freshness blocks for every SP in the build.
6. Build all three tiers: Tier 1 best standalone, then run tools/ticket.py with EVERY gate-clearing leg (TrueP:price:game[:label[:tier]]; declare a corr tier on deliberate same-game stacks) — Tier 2 = its best-floor pick, Tier 3 = its +180..+260 band pick. Paste its frontier into the build so the floor cost of the band is visible; never hand-pick a 3rd leg the search didn't rank first.
7. Pick the bankroll bet (single safest qualifying fav, independent of parlay, not A-list fade).
8. Append this run to parlays/YYYY-MM-DD.md as '## Run 11:00 ET — Build A'.
9. Log the WHOLE value scan in results_log.md — a row for EVERY candidate the scan surfaced (bet or not) with pre-registered TrueP + no-vig ImplP + Edge + the [adj: ...] tag from truep.py (this 4-5x's the calibration sample for free, per CLAUDE.md).
10. ODDS API CREDITS: run tools/odds_api.sh quota (FREE — 0 credits) and add a line 'Odds API credits remaining: <N> (used <M> this month)' to this run's section in today's parlay file BEFORE committing so it's captured in the repo record.
10b. DASHBOARD: run 'python3 tools/generate_dashboard.py' to refresh docs/index.html from the updated ledgers, and stage docs/index.html in the same commit (GitHub Pages auto-deploys it on merge to main).
11. Commit → push → open PR → squash-merge → git fetch+reset to main per CLAUDE.md git workflow.
12. Push notification: load PushNotification via ToolSearch (if needed), send title 'MLB Parlay Build A — <today>' summarizing Tier 1 standalone leg+edge, Tier 2 floor%, Tier 3 combined odds, bankroll bet, Odds API credits remaining; flag PENDING legs. APPEND the NRFI/YRFI line from 'python3 tools/nrfi_digest.py <today> --compact' (today's daily win% + NRFI/YRFI pick counts).
13. Email: load mcp__Gmail__create_draft via ToolSearch (if needed), draft to icecold67@live.com subject 'MLB Parlay — <today YYYY-MM-DD> Build A', body = the 3 tiers (legs+prices+edges/floor%) + bankroll bet + Odds API credits remaining + any PENDING flags (prose under 250 words), THEN append a '--- NRFI/YRFI <today> ---' section = the full output of 'python3 tools/nrfi_digest.py <today> --format md' (daily win% header + the day's picks table). The table is in ADDITION to the word limit."
  ;;

16|15)
  LABEL="16:00 ET — CLV capture + lineup lock + build update + user-angle execution"
  PROMPT="Run the 16:00 ET MLB parlay update per CLAUDE.md (2nd of 3 runs: 11:00 / 16:00 / 18:00 ET). This is the lineup-lock + CLV + user-angle run; it UPDATES the 11:00 Build A. Note: the early (~13:35 ET) games are already IN PROGRESS at 16:00 — mark them live-only (their Angle A live-ML is happening now). The session_start.sh digest is already in your context (injected by hook). Steps:
0. MISSED-RUN WATCHDOG: if today's parlays/YYYY-MM-DD.md does NOT contain a '## Run 11:00 ET' section (or the file doesn't exist), the morning firing was dropped — ABSORB Build A's duties FIRST (settle yesterday + prior-day full-slate review + calibration + slate-wide scan + initial 3-tier build + bankroll bet), open the file with '⚠ 11:00 RUN MISSED — absorbed at 16:00' so the gap is visible in the audit trail, and say so in the notification+email. (A scheduler firing can silently drop — 8/7 proved it; the next run self-heals, never assumes.)
1. CLV is AUTO-APPLIED by session_start.sh (clv_capture.py --apply) when the ET hour is 16-19, so today's open ML + game-total + run-line legs (cached slate, 0 credits) AND, on the paid tier, every counting-prop leg (K/hits/TB/HR/RBI/runs/SB/BB/hits-allowed/outs/ER via the live props feed) should already have a CLV verdict written. VERIFY the CLV column filled; if any covered row is still blank, run 'python3 tools/clv_capture.py --apply' and confirm. HEED any '⚠ EDGE GONE' warnings it prints — a leg whose edge evaporated at the close must NOT be (re)bet/locked. Then re-run tools/calib.py to reconcile. Then re-run 'python3 tools/pulse.py' — if its actions changed since 11:00 (a dimension cooled/suspended/shaded), apply them to every leg being locked or repriced NOW. Only team totals + live-ML stay manual.
2. Run 'python3 tools/recheck.py today's-date' (diffs live probables/status vs the 11:00 snapshot) — any ⚠ SP CHANGE or ⛔ started flag INVALIDATES every leg built on that arm/game (E3/E4 gate): re-run SP-freshness for the new arm or drop the leg before locking. Then tools/mlb_api.sh lineups today — for any leg still flagged PENDING LINEUP, check if lineups are now posted and upgrade or flag accordingly.
3. Run tools/mlb_api.sh ump today and tools/mlb_api.sh weather today — update any K-Over/Under or total legs affected by ump or weather changes.
4. Run tools/odds_api.sh best h2h today — check for material line moves on active legs.
4b. USER-ANGLE EXECUTION (results_log.md -> 'User-angle tracking' — directional-only, N<20, do NOT size off them):
   - Angle B (opposing-SP hits-allowed Over): for each WATCH candidate run tools/odds_api.sh events today for the eventId, then tools/odds_api.sh props <eventId> pitcher_hits_allowed (fall back to a manual book pull if the market is unsupported on tier). Devig with tools/devig.sh. Also pull props <eventId> pitcher_outs to gauge the quick-hook / start-length left tail. Log the real line + devigged edge into the Angle B table; flag hook risk.
   - Angle A (live-ML re-entry): for each WATCH candidate confirm the pregame ref ML (tools/odds_api.sh best h2h) and record the in-game TRIGGER (strong team trails early -> live price overshoots). Live-ML is placed in-game ONLY, so note the trigger + ref price; do not 'lock' it pre-game.
4c. FULL PROP VALUE SWEEP (the 16:00 run owns this — lineups are locked; quota-gate: only if the digest shows ODDS_MODE=rich). For each pre-game (not yet started) game with a confirmed lineup: tools/odds_api.sh events today for the eventId, then tools/odds_api.sh props <eventId> core (HR/TB/HRR/RBI + K/hits-allowed/ER, ~7cr) — or 'all' (~14cr) on games already showing a signal. For any K-leg candidate, price it with 'python3 tools/kprice.py <pitcher>' (~2cr: every K line + alternates, best-price devigged per line) instead of eyeballing one book. Devig EACH two-sided prop with tools/devig.sh, apply the min-edge gate (+2pp standalone / +3-4pp anchor), and run the prop-specific value reads in CLAUDE.md 'Full prop universe' (HR=park/wind/SP-HR9, TB/HRR=slot+matchup, RBI/runs=team-total correlated, SB=speed-vs-pop-time). Surface clearing props into the scan + log to results_log.md. Budget: prefer 'core' slate-wide or 'all' on ~3-5 signal games; do NOT sweep all-markets×all-games.
4d. NRFI/YRFI re-check (nrfi_tracker.md — MANDATORY every run): with lineups now locked, re-verify today's NRFI/YRFI leans (a scratched/rested leadoff bat flips a read) and adjust each TrueP. Today's early-game reads are AUTO-SETTLED by session_start.sh (nrfi_settle.py --apply); run 'python3 tools/nrfi_settle.py <today> --apply' to catch any game that finished since session start, and verify the record. For any read worth betting, pull the real 1st-inning total (tools/odds_api.sh props <eventId> totals_1st_1_innings, or a book) + devig — promote only if it clears +2pp. Keep the tracker rows current so the dashboard refreshes.
5. If any leg or price changed materially since the 11:00 build, APPEND a new '## Run 16:00 ET — Build B' section to today's parlays/YYYY-MM-DD.md (do NOT overwrite Build A — mark old legs SUPERSEDED if replaced).
6. Update results_log.md with new or revised legs per the SUPERSEDE protocol.
7. Confirm or update the bankroll bet if lineup was PENDING at 11:00.
8. ODDS API CREDITS: run tools/odds_api.sh quota (FREE — 0 credits) and add 'Odds API credits remaining: <N> (used <M> this month)' to this run's section in today's parlay file BEFORE committing.
8b. DASHBOARD: run 'python3 tools/generate_dashboard.py' to refresh docs/index.html from the updated ledgers, and stage it in the same commit (GitHub Pages auto-deploys on merge).
9. Commit → push → open PR → squash-merge → reset per CLAUDE.md git workflow.
10. Push notification: load PushNotification via ToolSearch (if needed), send title 'MLB Parlay Build B update — <today>': CLV fills (count +/−), ANY ⚠ EDGE-GONE legs (name them — user must not bet those) and ⚠/⛔ recheck flags (SP scratches / started games), lineup upgrades, whether the build changed, Odds API credits remaining. APPEND the NRFI/YRFI line from 'python3 tools/nrfi_digest.py <today> --compact'.
11. Email: load mcp__Gmail__create_draft via ToolSearch (if needed), draft to icecold67@live.com subject 'MLB Parlay — <today YYYY-MM-DD> Build B update', body = CLV fills per open leg, PENDING→CONFIRMED upgrades, superseded legs, current active build, Odds API credits remaining (prose under 200 words), THEN append a '--- NRFI/YRFI <today> ---' section = the full output of 'python3 tools/nrfi_digest.py <today> --format md'. The table is in ADDITION to the word limit."
  ;;

18)
  LABEL="18:00 ET — final CLV + late lineups + line check + live-ML settle"
  PROMPT="Run the 18:00 ET final MLB parlay check per CLAUDE.md (3rd of 3 runs: 11:00 / 16:00 / 18:00 ET). This is the late/west-coast lock + final CLV + live-ML trigger settle. The session_start.sh digest is already in your context (injected by hook). Steps:
0. MISSED-RUN WATCHDOG: if today's parlay file has no '## Run 16:00 ET' section, the 16:00 firing was dropped (8/7: it happened) — ABSORB Build B's duties in this run (verify/apply CLV, recheck.py diff, lineup lock, user-angle execution, and the FULL PROP VALUE SWEEP if ODDS_MODE=rich, since the 16:00 run owns that sweep and nobody else will run it), open this run's section with '⚠ 16:00 RUN MISSED — absorbed at 18:00', and say so in the notification+email. If '## Run 11:00 ET' is ALSO missing, additionally settle yesterday + run the scan + build from scratch. A missed firing must cost one flagged catch-up, never a silent hole in CLV/props/settles.
1. CLV auto-applied by session_start.sh (clv_capture.py --apply, ET hour 16-19) for ML + game totals + run lines (cached slate, 0 credits) and, on the paid tier, all counting props (live props feed). VERIFY today's open covered legs have a CLV verdict; run 'python3 tools/clv_capture.py --apply' for any still-blank late/west-coast legs now that their close is set. HEED '⚠ EDGE GONE' warnings — do not (re)bet those legs. Re-run tools/calib.py to reconcile. Re-run 'python3 tools/pulse.py' and apply any changed actions before the final lock.
2. Run 'python3 tools/recheck.py today's-date' — final SP-scratch/status diff vs the morning snapshot; ⚠/⛔ invalidates dependent legs (E3/E4 gate). Then tools/mlb_api.sh lineups today — confirm late/west-coast game lineups; upgrade any remaining PENDING legs to CONFIRMED or flag them.
3. Run tools/odds_api.sh best h2h today — check for any late sharp line moves.
3b. USER-ANGLE EXECUTION (results_log.md -> 'User-angle tracking' — directional-only, N<20): finish any Angle B (opposing-SP hits-allowed Over) line pulls for late/west-coast games (props <eventId> pitcher_hits_allowed + pitcher_outs, devig); for live games already underway, settle any Angle A live-ML triggers that fired and record the live price taken vs the pregame ref (the 'live CLV'). Update both Angle tables.
3c. NRFI/YRFI settle (nrfi_tracker.md — MANDATORY every run): session_start.sh auto-settled today's finished games (nrfi_settle.py --apply); run 'python3 tools/nrfi_settle.py <today> --apply' once more to settle any game that finished since, then VERIFY the W/L stamps + record and update the 'Settled'/'Open' prose lines. Games still in progress stay TBD (the next run settles them). This guarantees the tracker (and the dashboard NRFI list) is current at the end of every day.
4. If anything materially changed since the last build, APPEND '## Run 18:00 ET — Build C' to today's parlay file (mark prior build SUPERSEDED if replaced).
5. Flag any legs the user should manually re-check at first pitch.
6. ODDS API CREDITS: run tools/odds_api.sh quota (FREE — 0 credits) and add 'Odds API credits remaining: <N> (used <M> this month)' to this run's section in today's parlay file BEFORE committing.
6b. DASHBOARD: run 'python3 tools/generate_dashboard.py' to refresh docs/index.html from the updated ledgers, and stage it in the same commit (GitHub Pages auto-deploys on merge).
7. Commit → push → open PR → squash-merge → reset per CLAUDE.md git workflow.
8. Push notification: load PushNotification via ToolSearch (if needed), send title 'MLB Parlay Final check — <today>': final build legs confirmed, ANY ⚠ EDGE-GONE legs (name them — user must not bet those) and ⚠/⛔ recheck flags (SP scratches / started games), late lineup updates, first-pitch re-check flags, Odds API credits remaining. APPEND the NRFI/YRFI line from 'python3 tools/nrfi_digest.py <today> --compact' (by 18:00 most games are settled, so this carries the day's final win%).
9. Email: load mcp__Gmail__create_draft via ToolSearch (if needed), draft to icecold67@live.com subject 'MLB Parlay — <today YYYY-MM-DD> Final check', body = late lineup confirmations, remaining CLV fills, final active build legs+prices, first-pitch re-check flags, Odds API credits remaining (prose under 200 words), THEN append a '--- NRFI/YRFI <today> ---' section = the full output of 'python3 tools/nrfi_digest.py <today> --format md' (the day's final win% + picks table). The table is in ADDITION to the word limit."
  ;;

*)
  echo "ERROR: Unknown build type '$BUILD'. Use 11, 16, or 18 (aliases 09/15 accepted)." >&2
  exit 1
  ;;
esac

# ── Prompt-only mode (single source of truth for the hook + GitHub Actions) ───
# Pass --prompt-only to print JUST the prompt and exit (no header, no claude call).
# The session-start hook consumes this so the build prompts live in ONE place.
if [[ "$PROMPT_ONLY" == "1" ]]; then
  echo "$PROMPT"
  exit 0
fi

echo "[cron_build.sh] $(TZ=America/New_York date '+%Y-%m-%d %H:%M %Z') — $LABEL"

# ── Invoke Claude non-interactively ──────────────────────────────────────────
# NOTE: `claude -p` only works when called OUTSIDE a Claude Code session
# (from your cron/machine/GitHub Action). Calling it from inside an active
# session will fail — that's expected, not a bug.
if command -v claude >/dev/null 2>&1; then
  echo "[cron_build.sh] Invoking: claude -p ..."
  exec claude -p "$PROMPT"
else
  echo ""
  echo "ERROR: 'claude' CLI not found on PATH." >&2
  echo "  Option A — install Claude Code CLI: https://claude.ai/code" >&2
  echo "  Option B — GitHub Actions: use --prompt-only to get the prompt string:" >&2
  echo "    bash tools/cron_build.sh $BUILD --prompt-only" >&2
  echo ""
  echo "═══════════════════════════════════════════════════"
  echo "PROMPT FOR: $LABEL"
  echo "═══════════════════════════════════════════════════"
  echo "$PROMPT"
  echo "═══════════════════════════════════════════════════"
  exit 2
fi
