...
1. CLV is AUTO-APPLIED by session_start.sh (clv_capture.py --apply) when the ET hour is 15-19, so today's open ML + game-total + run-line legs should already have a CLV verdict written (cached slate, 0 credits). VERIFY the CLV column filled; if any covered row is still blank, run 'python3 tools/clv_capture.py --apply' and confirm. HEED any '⚠ EDGE GONE' warnings it prints — a leg whose edge evaporated at the close must NOT be (re)bet/locked. Then re-run tools/calib.py to reconcile. K/hitter props + team totals stay manual.
...
1. CLV auto-applied by session_start.sh (clv_capture.py --apply, ET hour 15-19) for ML + game totals + run lines (cached slate, 0 credits). VERIFY today's open covered legs have a CLV verdict; run 'python3 tools/clv_capture.py --apply' for any still-blank late/west-coast legs now that their close is set. HEED '⚠ EDGE GONE' warnings — do not (re)bet those legs. Re-run tools/calib.py to reconcile.
