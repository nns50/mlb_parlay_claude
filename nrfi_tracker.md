...
| 8/12 | PHI @ STL (**Wheeler 0.98** / Leahy 1.34) | NRFI | 58% | _model-only_ | TBD | Strongest read of the day. Wheeler's 0.98 WHIP / 10.77 K9 over 19 GS is the best 1st-inning profile on the board. ⚠ Held under 60% by Leahy's 1.34 and by 94°F / 6 mph out to RF at Busch. |
...
| 8/12 | TB @ ATH (**Rasmussen 0.91** / Perkins 1.45) | NRFI | 53% | _model-only_ | TBD | Board's best WHIP vs the majors' most hitter-friendly park and a 7.04 ERA opponent. Genuinely two-sided — barely above the line by design, not confidence. |
...
| 8/12 | BAL @ MIN (Baz 1.32 / **Matthews 1.34**) | YRFI | 52% | _model-only_ | TBD | Raw YRFI 57% → 52%. Two 1.3+ WHIPs; Matthews 5.23 ERA is the softest qualified starter of the early window. ⚠ Counter: 82°F, wind 5 mph IN from CF. |
| 8/12 | HOU @ SF (**King — opener, 0 GS / 49.1 IP** / Houser 1.37) | YRFI | 51% | _model-only_ | TBD | ⚠ **E5 — Houston is a BULLPEN GAME.** An opener's 1st inning is the least predictable on any board and cuts both ways, so the number is flattened rather than pushed. Oracle Park is the strongest suppressant venue on the slate. Re-read at 16:00. |
...
- **Record:** **187-156** (tracker opened 6/10/26). **NRFI:** **94-81** · **YRFI:** **93-75**. _(**8/9 is now FULLY SETTLED — 15 of 15 reads decided, and the day finished 10W-10L (50%)**; the five rows still open at the 18:30 Build C lock were auto-stamped off `linescore.innings[0]` at session start — DET@SF **W**, TB@SEA **L**, LAD@AZ **L**, CLE@CWS **L**, HOU@SD **L**. 8/8's 11-3 remains the tracker's best slate.)_
- **Staked:** $0 · **P/L:** $0.00 (model leans only — no priced bets yet; tracking calibration).
- **Open:** **8/12 — 15 reads logged, all model-only, all TBD** (8 NRFI / 7 YRFI after the correction; raw split is 7/8, so the ratchet reversed exactly one read — **NYM @ ATL**, marked for row-level grading and named in advance as the row most likely to be wrong). ⚠ **Three data traps carried into the reads rather than hidden:** **HOU @ SF is a Houston bullpen game** (Bryan King, **0 GS / 49.1 IP** — E5) and its read is deliberately flattened because an opener's first inning cuts both ways; **CHC @ WSH's Jackson Kent has no 2026 StatsAPI pitching record at all**, so half that read is missing and it too is flattened rather than guessed; and **KC @ LAD's Daniel Lynch IV shows a 1.01 WHIP that is relief data** (1 GS across 47.2 IP) and is not used as a starter's number. Strongest read either way is **TEX @ LAA YRFI 58%** off George Klassen's **2.88 WHIP over 8.2 IP** — roughly three baserunners per inning, the most extreme figure on the board — with the honest caveat that 8.2 innings is a tiny sample. No priced `totals_1st_1_innings` line pulled at 11:00; the sweep is the 16:00 run's duty.
