...
## Tonight's reads — 2026-08-26 (15-game slate; model-only at 11:00 — the priced `totals_1st_1_innings` sweep is the 16:00 run)

> ### ⚙ 8/25 — NO READS LOGGED (session ran but no 8/25 NRFI section was written — gap acknowledged)
>
> 8/25 results (linescore not pulled): 15-game slate settled. Running record unchanged at 275-229 (55%).
>
> ⚠ **KC@TOR is E5-gated (Spencer Miles IP/GS = 20.0) — reads the same as a bullpen-game note; 1st-inning model has no real starter on the TOR half. YRFI lean by default but not pre-registered.**

| Game | Lean | TrueP | Why |
|---|---|---|---|
| **KC @ TOR (Dobnak/Miles)** | **YRFI** | **62%** | ⭐ **Board's strongest YRFI lean.** ⛔ **E5 on Miles (IP/GS = 20.0 — pure opener).** Rogers Centre retractable dome removes weather. An opener's first inning against KC's top-of-order is the clearest YRFI setup on the board — fresh arm, max effort, but unfamiliar hitters and opener-game variance. ⚠ LOW-CONFIDENCE by construction: E5 = opener, and openers' 1st-inning profiles have wide variance. |
| **HOU @ NYY (Lambert/E.Rodríguez)** | **YRFI** | **61%** | **Elmer Rodríguez returned from 2+ months IL (last MLB start: 6/21).** Rust + Yankee Stadium hitter environment + NYY's loaded top-of-order (Judge, Soto) = live first-half YRFI. Lambert 3.11/1.15 recent form argues the other way but does not contain NYY's 1-2-3. |
| **PHI @ SEA (Luzardo/Woo)** | **NRFI** | **62%** | ⭐ **Board's strongest NRFI lean.** Luzardo 3.23 ERA / 11.19 K/9 and Woo (solid, T-Mobile arm) face each other inside **T-Mobile Park — the most run-suppressing park on the board**, retractable roof. Both arms have low-contact profiles. PHI's top is solid but not a murderous lineup. |
| **TB @ DET (Rasmussen/Melton)** | **NRFI** | **61%** | Rasmussen 3.01/0.95 WHIP + **Melton 1.70 ERA** — two of the best WHIPs on today's slate — at **Comerica Park** (spacious, pitcher-friendly). 75°F, 14 mph R→L (crosswind, neutral). Both tops are modest (TB and DET's lineups are not run-producing machines). |
| **MIN @ ATH (Prielipp/Ginn)** | **YRFI** | **59%** | Both young arms with unknown first-inning profiles at **Sutter Health Park (Sacramento, hitter-friendly/small)**. Prielipp is a rookie; Ginn has limited MLB experience. One or both halves is live for early contact. |
| **COL @ WSH (Gordon/Waldron)** | **YRFI** | **57%** | **Tanner Gordon** (COL) is one of the worst arms on the board. Playing at Nationals Park (not Coors), but Gordon's ERA carries the read — Colorado arms give up early traffic. Waldron is a submarine pitcher who suppresses the other half, keeping this at 57% rather than higher. |
| **TEX @ CWS (Gore/Burke)** | **YRFI** | **57%** | **Sean Burke** (CWS) is a young arm with limited MLB sample. **Rate Field** is a hitter-friendly park. Gore (TEX) is solid and suppresses his half; Burke's half is the YRFI driver. |
| **MIL @ NYM (May/Stock)** | **YRFI** | **57%** | **Robert Stock (NYM) 5.40 ERA / 1.86 WHIP across 3 GS (avg 4.1 IP)** — today's worst confirmed arm. MIL's top (Cain, Contreras, Yelich order) can cash in the first if Stock is uncomfortable. May (MIL) is a competent suppressor on his half, keeping this at 57% not higher. |
| **CHC @ AZ (Boyd/E.Rodriguez)** | **YRFI** | **56%** | Boyd (CHC) has struggled recently (ERA inflated, limited deep starts) and Eduardo Rodriguez (AZ) has been mediocre. Chase Field retractable dome takes weather out. Two middling arms = both halves are live. |
| **CIN @ SF (Lodolo/Roupp)** | **NRFI** | **58%** | **Oracle Park** is one of the two most suppressing parks on the board. Lodolo (3.28 ERA, clean arm) and Roupp (decent) face each other in pitcher-favorable conditions. CIN's top is not strong. |
| **PIT @ SD (Chandler/Vásquez)** | **NRFI** | **59%** | **Petco Park** — the second-most suppressing park on today's board. Chandler (PIT) is young but has shown solid early-inning stuff; Vásquez (SD) is reliable at Petco. Pittsburgh's top is one of the weakest in the NL. |
| **BAL @ STL (Bradish/McGreevy)** | **NRFI** | **57%** | Bradish is returning from TJ surgery but has pitched this season (verified: active roster, confirmed starter). McGreevy is a reliable arm. Busch Stadium is neutral-to-pitcher-leaning. STL's top is limited offensively. |
| **BOS @ MIA (Gray/Gusto)** | **NRFI** | **57%** | **Sonny Gray 2.79 ERA** (elite) is a strong first-inning suppressor on his half. Gusto (MIA) is a question mark that keeps the lean from being higher — if MIA runs a mediocre arm, BOS's top can score. loanDepot dome removes weather. |
| **CLE @ LAA (Cantillo/G.Rodriguez)** | **NRFI** | **56%** | Cantillo 2.54/1.03 WHIP (elite suppressor) anchors the NRFI lean. G.Rodriguez' season ERA (6.41) is stale-high (last start 8/20: 7 IP/0 ER — E2 gate). If his recent form holds, NRFI is higher. If the season number is real, it's a toss-up. Split at 56%. |
| **LAD @ ATL (Sasaki/Smith-Shawver)** | **NRFI** | **55%** | **Roki Sasaki** is an elite arm (3.15 ERA / 10.8 K/9) — his half is a genuine NRFI suppressor. **Smith-Shawver's 6.23 ERA (2 GS)** is the YRFI factor: LAD's top-of-order (Ohtani, Betts, Freeman) against a shaky arm at Truist Park. Slight NRFI edge from Sasaki's dominance. ⚠ Only 2 GS sample on Smith-Shawver — his actual first-inning profile is unknown. |

---

## Tonight's reads — 2026-08-24 (10-game slate; model-only at 11:00 — the priced `totals_1st_1_innings` sweep is the 16:00 run)

> ### ⚙ 8/23 CLOSED AT 8-7 (53%) — VERIFIED
>
> `nrfi_settle.py --apply` stamped all 15 8/23 rows off `linescore.innings[0]` at session start; verified present.
> **Running record 275-229 (55%)** (NRFI 133-110 · YRFI 142-119). 8/23 split: **NRFI leans 4-3, YRFI leans 4-4.**
> The board's strongest lean (CHC@SEA NRFI 62%, two aces at T-Mobile) **LOST** — SEA scored in the 1st and CHC
> went on to hang 19; a suppressing park does not guarantee a quiet top of the 1st. `type:nrfi` in the pulse
> window is now 3-2 (60%) with CLV 0+/0− — still never once measured against a close.
>
> ⚠ **Registry state unchanged:** `wind_out_over` RETIRED (0.0pp), `hitter_park_over` HALVED at +1.5, the −5pp
> YRFI correction RETIRED. GLOBAL SHRINK is armed on the main build; NRFI reads here are raw model, no correction.
> ⚠ **CIN @ SF is E4 on the home half — San Francisco's probable is TBD** — so that read is one-sided (Burns only)
> and marked LOW CONFIDENCE. Weather PENDING at all ten parks at 11:00 (three retractable-roof: MIA, AZ, SEA).

| Game | Lean | TrueP | Why |
|---|---|---|---|
| PHI @ SEA (Wheeler/Gilbert) | **NRFI** | **63%** | ⭐ **Board's strongest lean.** The two best WHIPs on the slate face each other — **Wheeler 1.01 / 10.77 K/9** and **Gilbert 1.01 / 9.40 K/9** — inside **T-Mobile Park, the most run-suppressing venue on the board**, retractable roof. Both aces routinely retire the top of the 1st in order. |
| MIN @ ATH (Z.Matthews/Springs) | **YRFI** | **60%** | **Two of the worst arms on the board** — Matthews 5.12/1.33, Springs 6.02/1.48 — at **Sutter Health Park (Sacramento), a small, hot hitter's park.** Both first innings are live; Springs' 1.48 WHIP is pure early-traffic. |
| CHC @ AZ (Gausman/M.Kelly) | **YRFI** | **59%** | **Two mediocre arms and a loaded Chicago top.** Gausman 4.58 (splitter, homer-prone) last two 4.2/4.1 IP with 9 ER; **Kelly 5.37 / 1.52** hit for 6 ER in his last start. Chase Field roof takes the weather out; the arms carry it. |
| TEX @ CWS (Rocker/Urquidy) | **YRFI** | **58%** | **Urquidy is freshly back with a 16-IP season sample** (one stretch 8.53 ERA / 2.68 WHIP) and Rocker's 4.46 / 1.40 walks people. **Rate Field is a hitter's park.** Both halves have a live path to a run. |
| COL @ WSH (Feltner/Cavalli) | **YRFI** | **57%** | **Feltner 5.97 / 1.53** is the worst season line among today's real starters — his half is a YRFI engine. Cavalli 3.22 argues the other way (which is why this is 57%, not higher), and Colorado's top is quiet on the road. |
| CLE @ LAA (Messick/Klassen) | **YRFI** | **56%** | **Messick 2.54 / 1.03 is elite early**, but **George Klassen (rookie, 5 GS) carries a 1.62 WHIP** — free baserunners in the 1st are the purest YRFI input. One-sided read: Messick's half suppresses, Klassen's leaks. |
| TB @ DET (Rasmussen/Valdez) | **NRFI** | **57%** | **Rasmussen 3.01 / 0.95** is the best WHIP on the board and Valdez is a sinker/ground-ball arm (weak early contact) despite a 1.40 WHIP. **Comerica Park** is spacious. Both tops modest. |
| PIT @ SD (Ashcraft/R.Ray) | **NRFI** | **57%** | **Petco Park**, the second-most suppressing venue on the board. Ashcraft 3.68 / 1.08 and Ray 3.20 (both solid); Pittsburgh's top is one of the weakest in the league. Ray walks people, which caps this at 57%. |
| BOS @ MIA (Suarez/Alcantara) | **NRFI** | **56%** | **Two steady arms** — Suarez 3.41 / 1.18, Alcantara 3.46 / 1.18 — inside **loanDepot park (dome, pitcher-leaning).** Boston's top is the counterweight that keeps this at 56% rather than higher. |
| CIN @ SF (Burns/TBD) | **NRFI** | **55%** | ⚠ **E4 — San Francisco's probable is TBD**, so half the read is unmodelled. **Chase Burns 2.51 / 1.11 with 10.52 K/9** is a genuine first-inning suppressor and **Oracle Park** is a pitcher's park; the read rests on his half alone. LOW CONFIDENCE. |

---

## Tonight's reads — 2026-08-23 (15-game slate; model-only at 11:00 — the priced `totals_1st_1_innings` sweep is the 16:00 run)

> ### ⚙ 8/22 CLOSED AT 7-6 (54%) — VERIFIED
>
> `nrfi_settle.py --apply` stamped **13** 8/22 rows off `linescore.innings[0]` at session start; the stamps were
> **verified present** before this section was written. **Running record 267-222 (55%)** (NRFI 130-107 · YRFI 137-115).
>
> ⚠ **One 8/22 read was priced and it CASHED: CLE @ COL 1st-inning OVER 0.5 at −140 (FanDuel)**, no-vig 56.7% vs a
> 58.2% model — a +1.5pp read that was correctly **not promoted** to a bet (it never cleared +2pp) and won anyway.
> `type:nrfi` in the pulse window is now **2-2 (50%) vs a 59% claim**. The pricing gate is still doing more work
> than the model is.
>
> ⚠ **Today's board is structurally unusual and it changes how these reads should be read: FIVE of fifteen games are
> bullpen games** (DET, KC, CWS, SF, ATH and PIT all announce openers or 0-3-start arms). A first-inning model built
> on starter quality has **no starter to model** in six of thirty half-innings. Those reads are marked LOW CONFIDENCE
> in the Why column rather than silently priced as if a real starter were pitching.
>
...
| CLE @ COL (Griffin/Sugano) | **YRFI** | **63%** | **Coors is the auto-YRFI tilt and nothing here fights it.** Sugano 4.70 / 1.26 was hit for **6 ER in 5.0 IP** in his last start; Griffin's 3.18 season line hides a **15.0 IP / 7 ER (4.20)** last three. Altitude plus the worse of the two 1st-inning profiles. |
| LAA @ TEX (Kikuchi/Quantrill) | **YRFI** | **60%** | **Kikuchi has not pitched since April 29** — a four-month absence — and his last logged line was **5.81 ERA / 1.58 WHIP with 13 BB in 31.0 IP**. First innings after a long layoff are where the rust shows. Quantrill's half argues the other way (last three 13.0 IP / 1 ER), which is why this is 60% and not higher. |
| WSH @ MIA (Kent/Junk) | **YRFI** | **60%** | **Jackson Kent has walked 8 in 7.2 major-league innings across two starts** (5 BB on 8/12, 3 BB on 8/18) at a **9.39 ERA / 2.48 WHIP** — free baserunners in the 1st are the purest YRFI input there is. loanDepot's dome pulls the number back down; on a neutral field this reads 64%. |
| CIN @ AZ (Abbott/Bratt) | **YRFI** | **59%** | Two of the three highest WHIPs among today's priceable starters — **Abbott 1.40**, **Bratt 1.47** — with Bratt coming off **3.0 IP / 3 ER / 3 BB**. Chase Field's roof takes the weather out of it, so this is a pure two-arms read. |
| TB @ BAL (N.Martinez/Baz) | **YRFI** | **59%** | **Nick Martinez's 3.05 season ERA is stale-LOW: his last three are 18.1 IP / 15 ER = 7.36**, including **3.2 IP / 6 ER** on 8/18 against this same Toronto-adjacent AL East chalk. Camden at **81°F with 10 mph out to RF** is the most Over-friendly air on the board, and Baz's 1.35 WHIP is no counterweight. |
| SF @ BOS (Wilkinson/Bennett) | **YRFI** | **58%** | **Matt Wilkinson has thrown 2.0 innings all season** (one appearance, 8/19) — San Francisco are opening a bullpen game, and an unknown arm facing Boston's top three at **Fenway** is a first-inning coin-flip at best. Bennett's 1.07 WHIP is the only thing holding this under 60%. |
| ATH @ HOU (Basso/Javier) | **YRFI** | **57%** | **Cristian Javier at 6.44 / 1.68** is the worst season line among today's real starters and he has not completed six innings since 8/3. Brady Basso is an opener (six straight outings of 1.0-2.1 IP), so both halves of the 1st are pitched by arms with no length. Roof closed at 72°F keeps it modest. |
| TOR @ NYY (Soriano/Rodón) | **YRFI** | **57%** | **Rodón's first start back from a seven-week absence went 4.0 IP** (6/28 → 8/18), and rust plus a loaded New York top-of-order on the other side gives both halves a live path to a run. The 7 mph R-to-L crosswind fires no tag either way. |
| PIT @ LAD (Bachar/Snell) | **YRFI** | **56%** | **Pittsburgh are running an opener** — Lake Bachar's last six outings are 3.0/2.0/2.0/3.0/3.0/1.0 IP — into the Dodgers' 1-2-3, which is the single worst first-inning matchup on the board. ⚠ Held at 56% rather than higher because **Snell (12.00 K/9 over his three starts back) is the counter-half**, and D5's rule applies here too: a bullpen game is not a free Over. |
| DET @ KC (Waguespack/Lynch IV) | **NRFI** | **57%** | **Both clubs are running bullpen games** — Waguespack has **0 starts in 27.2 IP**, Lynch IV **3 in 54.0** — and an opener's *first* inning is usually his best: fresh, max-effort, one time through the top. Kauffman at 3 mph is neutral air. ⚠ Low confidence by construction: neither arm has a first-inning sample worth the name. |
| STL @ PHI (Leahy/C.Sánchez) | **NRFI** | **56%** | **Cristopher Sánchez 2.51 / 1.22 with 10.41 K/9** is the best arm on the slate and his last three read 17.0 IP / 3 ER; Leahy's 3.24 / 1.31 has held opponents to 3 ER over his last three as well. ⚠ Pulled down from 60% by **80°F and 8 mph out to RF at Citizens Bank** plus Philadelphia's top-of-order — the park is the reason this is not the board's NRFI lean. |
| MIN @ SD (Ober/Buehler) | **NRFI** | **57%** | **Petco Park** is the second-most suppressing venue on the board and both tops are quiet — Minnesota are 63-67 with a −47 run differential. Neither arm is good (Ober 4.62, Buehler 4.79), but Buehler's last two are 6.0 IP / 1 ER and 6.0 IP / 2 ER and the park does the rest. |
| ATL @ MIL (Mahle/Drohan) | **NRFI** | **56%** | **Mahle's 4.53 season ERA is stale-HIGH — his last three are 18.2 IP / 3 ER = 1.45**, the best recent stretch by any starter on this board. Drohan 3.86 / 1.25 is steady. ⚠ Counter-signal: **Drohan's last outing was only 2.0 IP**, which is either a planned short start or something worse, and the 16:00 run should re-check it. |
| NYM @ CWS (McLean/Newcomb) | **NRFI** | **55%** | **McLean 3.34 / 1.16 with 10.22 K/9** is a genuine first-inning suppressor. ⚠ The other half is an opener — **Newcomb has 3 starts in 70.0 IP and his last four outings are 1.0/1.1/2.1/2.0 IP** — so this is a one-sided read at a hitter-friendly Rate Field, which is why it sits at the bottom of the NRFI group. |

---

## Tonight's reads — 2026-08-22 (15-game slate; model-only at 11:00 — the priced `totals_1st_1_innings` sweep is the 16:00 run)

> ### ⚙ 8/21 CLOSED AT 8-6 (57%) — VERIFIED, and the day's split contradicts the file's own bias
>
> `nrfi_settle.py --apply` stamped **14** 8/21 rows off `linescore.innings[0]` at session start; the stamps
> were **verified present** before this section was written. **Running record 249-211 → 259-215 (55%)**
> (NRFI 130-104 · YRFI 129-111).
>
> ⚠ **The board's strongest NRFI lean and its strongest YRFI lean BOTH cashed, which is the useful part.**
> ATL @ MIL (NRFI 63%, the two best WHIPs on the slate) finished its first inning **0-0 ✅**, and CLE @ COL
> (YRFI 64%, Coors plus the two worst WHIPs) scored **✅**. The tracker's conviction reads are carrying it;
> the sub-56% leans are where the record leaks.
>
> ⚠ **One read is separated because it was priced and correctly refused.** SF @ BOS NRFI was taken to a real
> `totals_1st_1_innings` market at 16:00 (**−155 BetRivers, no-vig 58.8% vs a 58.0% model**) and declined at
> **−0.8pp**. The first inning **scored**, so the decline saved the stake. `type:nrfi` in the pulse window is
> **1-2 (33%) vs a 59% claim** — the pricing gate is doing more work than the model is.
>
> ⚠ **Registry state unchanged:** `wind_out_over` RETIRED (0.0pp), `hitter_park_over` HALVED at +1.5, the
> −5pp YRFI correction RETIRED. Today's reads are **raw model, no correction**.
>
| Game | Lean | TrueP | Why |
|---|---|---|---|
| TOR @ NYY (Cease/Weathers) | **NRFI** | **64%** | ⭐ **Board's strongest lean in either direction, and the market agrees — 6.5 is the lowest full-game total on the slate by a full run.** **Cease 2.42 ERA / 1.06 WHIP / 13.14 K/9**, last three **18.1 IP / 5 ER with 27 K**; **Weathers 3.56 / 1.19 but last three 19.0 IP / 2 ER = 0.95**, the hottest stretch by any starter today. Add the only live weather read on the board — **74°F, wind 6 mph IN from CF** — and both halves point the same way. |
| ATL @ MIL (M.Pérez/Henderson) | **NRFI** | 62% | **Second-strongest, on the two best WHIPs facing each other.** **Henderson 2.70 / 0.87** — the best WHIP on the slate — last three 19.1 IP / 6 ER with 21 K; **Martín Pérez 3.15 / 1.19** with a 6.71 K/9 contact profile but last three **14.0 IP / 4 ER = 2.57**. Retractable roof removes weather from the read entirely. |
| MIN @ SD (Kremer/Mize) | **NRFI** | 55% | **Petco plus two arms whose recent form beats their season lines.** **Mize 3.18 / 1.07** and **Kremer's last three 17.1 IP / 6 ER = 3.12** against a 5.40 season ERA — E2 stale-HIGH on the Minnesota half. Held to 55 because Kremer's aggregate is genuinely bad and one bad first inning is all it takes. |
| TB @ BAL (McClanahan/B.Young) | **YRFI** | 54% | ⚠ **C3 is the whole read: McClanahan's last two outings are 3.0 and 3.1 IP after an 18-day gap.** A pitcher on a short leash coming off the IL concedes early traffic more often than his 3.25 / 1.12 line suggests, and **Young's last three (14.1 IP / 8 ER = 5.02)** are the worse half. Thin lean, logged for calibration. |
| DET @ KC (D.Anderson/Wacha) | **YRFI** | 54% | ⚠ **E5 — Detroit's Drew Anderson is a reliever mid-stretch-out** (5 GS across 76.1 IP; last outings 3.2, 4.0, 5.0). A converted arm's first inning is his least-tested inning. **Wacha 3.58 / 1.15** is the steadier half and holds the number down. Thinnest lean on the board. |
| ATH @ HOU (J.Lopez/H.Brown) | **YRFI** | 55% | **Houston's half is the read, and it is the reverse of the full-game market's assumption.** **Hunter Brown's last three are 16.0 IP / 8 ER = 4.50 with five walks in his most recent start** — walks are the single most reliable 1st-inning YRFI driver. Lopez's 5.22 / 1.53 season line supports it; his 3.00 last-three argues against. |
| PIT @ LAD (J.Jones/Skubal) | **YRFI** | 55% | **Jared Jones owns this read.** His 4.78 / 1.20 understates a **last three of 12.0 IP / 12 ER = 9.00**, including a 3.0 IP / 8 ER start on 8/9 — a collapse profile that starts early. **Skubal (2.85 / 0.96) is the best NRFI half on the board**, which is exactly why the number sits at 55 rather than 60. |
| WSH @ MIA (Irvin/E.Pérez) | **YRFI** | 56% | **Irvin's half carries it: 5.80 / 1.36, last three 14.2 IP / 11 ER = 6.75.** Eury Pérez (3.23 / 1.11, last three 2.00) is a strong NRFI half, so this is a one-sided read — and note it is the *same* asymmetry the main build priced into a Miami ML, which is the 8/20 lesson about 1st-inning and full-game reads being different objects. |
| NYM @ CWS (C.Scott/Castillo) | **YRFI** | 57% | **Both halves are worse than they look.** **Castillo 4.99 / 1.37**; **Scott's 3.51 season line is stale-LOW — last three 15.1 IP / 10 ER = 5.87**, with a 4.0 IP / 4 ER outing on 8/10. Two arms conceding early traffic at above-average rates. |
| CIN @ AZ (Lowder/Soroka) | **YRFI** | 57% | **Lowder 5.17 / 1.49** is the second-worst WHIP among today's named starters, and WHIP is the 1st-inning metric that matters. ⚠ **Soroka is two starts off a two-month IL absence** — a ramping arm's command is least settled in the first inning, so the Arizona half does not suppress this as much as his 2.93 ERA implies. |
| LAA @ TEX (R.Johnson/Bradford) | **YRFI** | 58% | **Ryan Johnson 6.41 / 1.54** is the worst combined line on the slate among arms with real volume. ⚠ **Bradford's 2.93 rests on 15.1 IP across 3 GS with a 1.43 WHIP inside that sample** — the ERA is not evidence, the WHIP is, and it points the same way. |
| STL @ PHI (TBD/Painter) | **YRFI** | 58% | ⚠ **E4 — St. Louis have not named a probable, so half this read is unresolved and the number is held short of the conviction band.** Philadelphia's half is unambiguous: **Andrew Painter 6.43 ERA / 1.55 WHIP over 16 GS** — the worst WHIP of any named starter today. |
| CHC @ SEA (Peterson/K.Anderson) | **YRFI** | 58% | ⚠ **E4-class: Kade Anderson returns a completely EMPTY 2026 StatsAPI line — an MLB debut.** A debut starter's first inning is the highest-variance inning on any board, and **Peterson's 5.29 / 1.53** supplies the other half. Logged as a model read; **do not price this one at 16:00 without the debut confirmed.** |
| SF @ BOS (Tidwell/Sandoval) | **YRFI** | 62% | **Second-strongest YRFI, and it is a WHIP read end to end. Sandoval's 1.85 WHIP is the worst on the slate by 0.31**, last three 15.0 IP / 11 ER = 6.60 including 4.0 IP / 7 ER on 8/16. Tidwell (1.22 over 3 GS) is thin evidence rather than a suppressor. Fenway adds nothing to suppress it. |
| CLE @ COL (Bibee/G.Hughes) | **YRFI** | **65%** | ⭐ **Board's strongest YRFI: Coors Field, the slate's highest total by 2.5 runs, and Gabriel Hughes' last three read 13.1 IP / 19 ER = 12.83.** Same construction that cashed here yesterday. Bibee (4.01 / 1.15) is the better half and his last three are still 5.50. |

**Model-only / NO BET on all fifteen** — no `totals_1st_1_innings` line was pulled at 11:00. The priced sweep
is the 16:00 run's job (a 1st-inning line before lineups post is not worth the credits), and a read is
promoted to a bet only if a real posted line leaves it clearing the **+2pp** gate. ⚠ **Three reads carry a
disqualified half and must not be priced at 16:00 without re-checking: STL @ PHI (E4, no probable), CHC @ SEA
(E4-class, Kade Anderson has no MLB record), DET @ KC (E5, Drew Anderson is a reliever mid-stretch-out).**
⚠ **The pulse window has `type:nrfi` at 1-3 (25%) against a 59% claim** — small n, but it is the reason the
pricing gate, not the model, decides whether any of these becomes a bet.

---
## Tonight's reads — 2026-08-21 (15-game slate; model-only at 11:00 — the priced `totals_1st_1_innings` sweep is the 16:00 run)

> ### ⚙ 8/20 CLOSED AT 6-3 (67%), AND THE TRACKER IS RE-BASED
>
> `nrfi_settle.py --apply` stamped all nine 8/20 rows off `linescore.innings[0]` at session start and the
> stamps were **verified present** before this section was written. **Running record 243-208 → 249-211 (54%)**
> (NRFI 125-102 · YRFI 124-109). The day's split: **NRFI leans 4-1, YRFI leans 2-2.**
>
> ⚠ **One read is worth separating because it was right for a reason the main build got exactly backwards.**
> LAA @ HOU was the tracker's strongest 8/20 lean — **YRFI 60%, on G-Rod's 7.17 ERA / 1.59 WHIP** — and the
> first inning went **1-2, YRFI ✅**. Meanwhile the parlay build fired a `~own_sp_hi+5` mirror onto Houston
> off **the same pitcher's same bad line**, and **G-Rod then threw seven shutout innings** while Houston's
> certified-clean starter allowed nine earned. **The 1st-inning read and the full-game read are different
> objects, and 8/20 is the cleanest demonstration of that this file has produced**: a 1.59 WHIP reliably
> concedes early traffic without saying anything durable about nine innings.
>
> ⚠ **Registry state unchanged from 8/19:** `wind_out_over` stays RETIRED (contributes 0.0pp), `hitter_park_over`
> stays HALVED at +1.5, the −5pp YRFI correction stays RETIRED. Today's reads are **raw model, no correction**.
>
| Game | Lean | TrueP | Why |
|---|---|---|---|
| ATL @ MIL (Sale/Misiorowski) | **NRFI** | 63% | ⭐ **Board's strongest NRFI lean, and it is not close: the two best WHIPs on the slate face each other.** **Misiorowski 1.75 ERA / 0.75 WHIP / 13.60 K/9** over 23 GS (last three 19.0 IP / 6 ER = 2.84) and **Sale 2.16 / 1.02 / 11.16 K/9** over 22 GS (last three 18.0 IP / 4 ER = 2.00, with 9 K in two of them). Whiffs are the cleanest 1st-inning suppressant there is and both halves have them. American Family roof is a dome input; weather PENDING at 11:00. ⚠ Counter: Milwaukee's top three are the best offence in the NL by run differential (+159). |
| STL @ PHI (**Dobbins**/Luzardo) | **NRFI** | **55%** ⬆ *(16:00: E4 RESOLVED)* | ⚠ **E4 — St. Louis have not named a probable, so half this read is unresolved and the number is deliberately close to a coin flip.** Philadelphia's half is the opposite of unresolved: **Luzardo 3.23 / 1.15 / 11.08 K/9**, and his last three are **21.2 IP / 3 ER = 1.25** with 12 K and 9 K games in them — stale-HIGH season line, dominant right now. Citizens Bank is a genuine hitter park pulling the other way. **The lean was on Luzardo alone at 11:00 and was flagged as such.** ⚙ **16:00 UPDATE — E4 RESOLVED, and the read moves only one point.** `recheck.py` reports St. Louis' probable posted: **Hunter Dobbins, 3.42 ERA / 1.30 WHIP / 8.82 K/9 over 50.0 IP, last three 16.1 IP / 5 ER = 2.76** — solid recent form, which is why this goes to **55% and not to 60%**. ⚠ **His 1.30 WHIP is the half that caps it**, and the 8/20 lesson in this file is exactly that a middling WHIP reliably concedes early traffic regardless of what the full-game line says. Citizens Bank remains a genuine hitter park pulling the other way. **Model-only — no `totals_1st_1_innings` market was priced for this game, so no bet.** |
| TOR @ NYY (Fluharty/Schlittler) | **NRFI** | 54% | ⚠ **E5 — Toronto's listed probable Mason Fluharty is a RELIEVER**: 3 GS across 51.0 IP, and every one of his last nine outings ran 0.1-1.0 IP. This is a bullpen game, so the away half is one fresh high-leverage arm for one inning rather than a starter settling in — genuinely ambiguous for a 1st-inning read, which is why the number sits at 54 and not higher. New York's half is clean: **Schlittler 2.19 / 0.94 / 11.19 K/9**, last three 15.1 IP / 6 ER (one 3.0/4 clunker bracketed by 7.0/1 with 11 K and 5.1/1 with 7 K). Yankee Stadium is a hitter park and the Toronto top three have an L10 of 7-3. |
| SF @ BOS (Webb/Gray) | **NRFI** | 58% ⛔ *(16:00: PRICED — NO BET)* | **Two arms in the best form on the board.** **Webb 3.50 / 1.06** with a ground-ball profile — weak early contact is the second-best 1st-inning suppressant after whiffs — and his last three are **20.0 IP / 2 ER = 0.90**. **Gray 2.65 / 1.15**, last three **19.0 IP / 2 ER = 0.95** including two scoreless outings. ⚠ Counter, and it is the reason this is 58 rather than 62: **Fenway is a hitter park** and Boston's top three are backed by a +95 run differential. ⛔ **16:00 UPDATE — PRICED AND DECLINED, which is the correct outcome rather than a failure.** This was the board's best *still-bettable* lean, so it was taken to a real market: **NRFI −155 (BetRivers) / YRFI +135 (BetOnline.ag), hold 3.4% → no-vig 58.8%.** Against a pre-registered **58%**, that is **edge −0.8pp — the market is priced AHEAD of the model**, and betting it would mean paying the book to agree with us. **NO BET.** ⚠ **The board's genuinely strongest lean, ATL @ MIL at 63%, was unbettable at this run entirely** — it went `In Progress` at 20:10Z, so the status gate closed on the one read worth chasing. That is a real timing cost of the 11:00-model / 16:00-price split and is recorded as such. |
| WSH @ MIA (Lord/Gusto) | **NRFI** | 53% | ⚠ **E5 — Washington's listed probable Brad Lord is a swingman**: 3 GS across 62.1 IP, recent outings 1.0, 2.2, 0.2 IP. Second bullpen game on the board. Miami's **Gusto 4.70 / 1.32** is a soft half on the season line, though his last three read **14.2 IP / 5 ER = 3.07**. **The lean rests almost entirely on the park** — loanDepot is a run-suppressor with the roof — rather than on either arm, which is a thin thesis and priced as one. |
| TB @ BAL (F.Peralta/T.Rogers) | **YRFI** | 60% | ⭐ **Strongest YRFI lean that is not Coors.** **Freddy Peralta's 5.27 / 1.48 understates how badly he is throwing**: his three starts since the trade read **14.1 IP / 12 ER = 7.53**, and the E2 check points the *wrong* way for him — the aggregate is stale-LOW, not stale-high. A 1.48 WHIP concedes 1st-inning traffic by construction. Baltimore's **Rogers 4.15 / 1.25** is the steadier half (last three 16.0 IP / 6 ER = 3.38). **Camden is a hitter park.** The read is Peralta's half plus the park. |
| NYM @ CWS (Manaea/Burke) | **YRFI** | 55% | **Burke's 3.15 / 1.11 / 9.90 K/9 season line is the better number and his recent form is not**: last three **15.2 IP / 7 ER = 4.02**, with 4.1-inning outings on either side of a 7.0/1 gem. **Manaea 4.22 / 1.32** is the soft half on the season, though he has been the steadier of the two lately (19.1 IP / 7 ER = 3.26, including 11 K at Pittsburgh). Rate Field is a mild hitter park. Thin lean, logged for calibration. |
| DET @ KC (Melton/Cameron) | **NRFI** | 62% | ⭐ **Second-strongest NRFI, and the cleanest recent-form pair on the board.** **Troy Melton 1.49 ERA / 0.96 WHIP** over 14 GS with a last three of **17.1 IP / 1 ER = 0.52**. **Noah Cameron's 4.16 season is stale-HIGH** — his last three read **22.2 IP / 5 ER = 2.03** and include a **9.0 IP shutout with 8 K on 8/16**. **Kauffman is a run-suppressing park.** ⚠ Counter: Kansas City ride a 5-game win streak, so the home top three are not cold. |
| ATH @ HOU (Ginn/Wesneski) | **YRFI** | 55% | **Houston's half is the read.** **Wesneski 5.16 / 1.32 across only 22.2 IP in four starts back from the IL**, last three **17.0 IP / 10 ER = 5.29** — the aggregate is confirmed by recent form rather than stale, and a post-IL arm's first inning is where the rust shows. Oakland's **Ginn 3.53 / 1.22** is the better half (last three 18.0 IP / 8 ER = 4.00, all three exactly 6.0 IP — steady). Daikin roof; weather PENDING. ⚠ Counter: the Athletics' top three belong to a 49-79 club with an L10 of 2-8. |
| LAA @ TEX (Detmers/Gore) | **NRFI** | 59% | **Both season lines are stale-HIGH and both arms are hot — the rare case where E2 fires in the same direction on both halves.** **Detmers 3.78 / 1.06 / 10.51 K/9**, last three **18.0 IP / 4 ER = 2.00**, capped by an **8.0 IP shutout with 11 K on 8/15**. **Gore 4.45 / 1.26 / 9.92 K/9**, last three **17.0 IP / 4 ER = 2.12**. Two double-digit-K/9 arms in a dome is a straightforward NRFI shape; Globe Life's roof removes the weather input. |
| CLE @ COL (Cantillo/Gordon) | **YRFI** | 64% | ⭐ **Board's strongest lean in either direction: Coors Field plus the two worst WHIPs on the slate.** **Cantillo 3.80 / 1.44** and **Gordon 5.55 / 1.44** — a 1.44 WHIP concedes roughly one-and-a-half baserunners an inning at sea level, and this is not sea level. Gordon's last three (6.0/4, 3.1/2, 4.0/0) also show a short leash. **The market has this at a 10.5 game total, the highest on the board by two runs**, which is the market agreeing with the direction; the read is logged model-only rather than bet because the park is exactly what a 1st-inning line already prices. |
| CIN @ AZ (Lodolo/E.Rodriguez) | **YRFI** | 56% | ⚠ **E2 fires against Arizona: Eduardo Rodriguez's 2.71 season line is stale-LOW.** His last three read **18.2 IP / 9 ER = 4.34**, and at **6.39 K/9** he is the lowest-strikeout arm on the board — contact-dependent arms give up 1st-inning runs. Cincinnati's **Lodolo 4.52 / 1.42** is the softer half on paper (last three 14.0 IP / 6 ER = 3.86). Chase Field roof; weather PENDING. |
| MIN @ SD (Prielipp/TBD) | **YRFI** | 55% | ⚠ **E4 — San Diego have not named a probable, so half this read is unresolved.** Minnesota's half is the whole lean and it is a poor one: **Prielipp 5.34 / 1.38** with a last three of **15.1 IP / 14 ER = 8.22**, including **8 ER in 5.0 IP on 8/15**. ⚠ Counter, and it is a real one: **Petco is one of the best pitcher parks in baseball**, which is why this sits at 55 rather than 60. |
| PIT @ LAD (Chandler/Yamamoto) | **NRFI** | 56% | **Yamamoto owns his half — 2.60 / 0.89, the second-best WHIP on the board, last three 19.2 IP / 4 ER = 1.83.** The number is held down to 56 by the other two inputs: **Chandler's 4.23 / 1.37 season line is stale-HIGH** (last three 16.2 IP / 4 ER = 2.16, so he is pitching well, but a 1.37 WHIP is the season-long traffic profile), and **the Dodgers' top three are the single most YRFI-prone trio in the league** at a +145 run differential. Two of the three drivers point NRFI, one points hard the other way. |
| CHC @ SEA (Boyd/Hancock) | **YRFI** | 54% | **Thinnest lean on the board, logged for calibration rather than conviction.** **Boyd's 4.02 / 1.24 is stale-LOW — his last three read 19.0 IP / 13 ER = 6.16**, capped by **7 ER in 5.1 IP on 8/15**, and Chicago's top three sit behind a +100 run differential. Against that: **Hancock 3.30 / 1.11** is steady (last three 17.2 IP / 7 ER = 3.57) and **T-Mobile Park suppresses runs** with the roof. The two halves genuinely cancel; the YRFI side wins on Boyd's form by a hair. |

**Model-only / NO BET on all fifteen** — no `totals_1st_1_innings` line was pulled at 11:00. The priced
sweep is the 16:00 run's job (a 1st-inning line before lineups post is not worth the credits), and a read
is promoted to a bet only if a real posted line leaves it clearing the **+2pp** gate. ⚠ **Two reads carry an
E4 half (STL @ PHI, MIN @ SD) and two carry an E5 bullpen-game half (TOR @ NYY, WSH @ MIA)** — those four are
logged as calibration rows but should not be priced at 16:00 without re-checking the resolved probable.

---
## Tonight's reads — 2026-08-20 (9-game slate; model-only at 11:00 — the priced `totals_1st_1_innings` sweep is the 16:00 run)

> ### ⚙ 8/19 CLOSED, TRACKER RE-BASED
>
> `nrfi_settle.py --apply` stamped the 10 finaled 8/19 rows off `linescore.innings[0]` at session start
> (5 W / 5 L on the day). **Running record 240-197 → 243-208 (54%)** (NRFI 122-100 · YRFI 121-108).
> The −5pp YRFI correction stays retired (RETIRED 8/19 at completed review — the reads below are RAW
> model with no correction applied).
>
> ⚠ **`wind_out_over` was retired from truep.py on 8/19** (n=28, 46%, skill −0.0025), so wind-out parks
> contribute 0.0pp to their YRFI reads today; `hitter_park_over` was HALVED to +1.5 the same run.
>
> ⚠ **`ace_edge` came OFF WATCH on 8/19** at n=20 unique / +0.0008 skill, so ace WHIP reads on 1st-inning
> quality carry their full magnitude. **`own_sp_hi` remains the best-measured tag on the board** (skill
> +0.0109, directional at n=12), and it fires cleanly today on G-Rod (7.17 ERA — LAA @ HOU YRFI half).
>
| Game | Lean | TrueP | Why |
|---|---|---|---|
| STL @ CIN (McGreevy/Singer) | **YRFI** | 55% | **GABP is a genuine YRFI park** and Singer's 4.66 / 1.44 season line supports it, though his last three are 6/3, 6/3, 6/3 — remarkably steady, not the 1st-inning volatility profile. McGreevy 3.48 / 1.22 with **6.20 K/9** limits damage but does not miss bats — that combination is the shape that gets touched early. Neither top-of-order is elite (STL +8, CIN −89). |
| SF @ CLE (Roupp/Williams) | **NRFI** | 58% | ⭐ **Williams' 11.48 K/9 over 149 IP is the highest strikeout rate on the slate** and whiffs are the cleanest 1st-inning suppressant there is. **Progressive Field is a genuine suppressant venue** and the wind is 7 mph IN From RF at 77°F — reinforcing. SF are 51-75, −74 differential, the worst offence on the board; Roupp's 1.30 WHIP is the drag, and last three at 5.75 ERA is why this is not 60%+. |
| TOR @ TB (Bieber/Seymour) | **NRFI** | 57% | Dome, no weather input. **Seymour's 10.74 K/9 owns the home half** and his last three are 6/2, 6/1, 6/3 — steady, no 1st-inning giveaways. Bieber's 4.99 / 1.56 season line is the counter but his last three are 3.64 (recent form BETTER than aggregate), so the shape argues Under. Neither top-of-order is an engine. |
| ATH @ KC (Jump/Dobnak) | **YRFI** | 55% | **Jump's 4.50 / 1.45 with 6/2 recent variance** (3.2/6, 6/2, 5.1/3) is the soft half — a 6.60 ERA over his last 15 IP includes the 8/2 blowup. Dobnak's 1.73 season is a **6-GS small-sample number** and his 1.30 WHIP concedes traffic; K/9 5.40 is the second-lowest on the board. Kauffman is neutral, wind 5 mph IN From CF at 79°F is a mild counter. |
| ATL @ CWS (Holmes/Kay) | **NRFI** | 55% | Two average-ish arms with steady recent form: **Holmes 3.82 / 1.35 last three at 3.45 (with 8/15 rough)**, **Kay 4.01 / 1.34 last three at 2.55**. Rate Field is neutral, wind 8 mph L To R at 74°F is a wash. Neither top-of-order is a dominant engine (CWS +47 is genuinely respectable this year). |
| SEA @ MIL (Kirby/Gasser) | **NRFI** | 58% | **Kirby 3.89 / 1.31 with 7.51 K/9 season** and last three at 3.24 — solid 1st-inning quality. **Gasser 4.54 / 1.24 with 8.35 K/9** and last three at 3.94 — decent WHIP is the operative number here. Dome, no weather input. ⚠ Note MIL scored 22 runs in this park two days ago; a 22-run game is a poor predictor of one inning but is the honest counter. |
| NYY @ BAL (Cole/Bradish) | **NRFI** | 59% | ⭐ **Cole's 1.08 WHIP over 86 IP is the best on the board today** and his last four (24.2 IP / 4 ER = 1.46 ERA) confirm it. **Bradish's 1.36 WHIP is workable** but his last four (19.2 IP / 10 ER = 4.57) argues traffic. Camden is a hitter park and the wind is PENDING — the counter-signal that keeps this at 59, not 62. |
| WSH @ TEX (Alvarez/deGrom) | **YRFI** | 57% | **deGrom's 3.95 season line is stale-LOW — his last three are 3.1/5, 5.0/1, 2.0/2 = 7.13 ERA and one 2-IP outing** (the classic short-leash 1st-inning red flag). Alvarez is the opposite (2.59 recent) so the whole read rests on Texas's half of the inning. Globe Life dome, weather PENDING. |
| LAA @ HOU (G-Rod/Lambert) | **YRFI** | 60% | ⭐ **Board's strongest lean. G-Rod's 7.17 ERA / 1.59 WHIP is the worst 1st-inning quality on the board** — his last three (7/2, 4/3, 4/3 = 4.80) are better than season but still poor. Lambert (3.11 / 1.15) owns his half, so the whole read rests on the top of Los Angeles's order finding the strike zone Grayson has been missing. Daikin roof — weather PENDING. |

**Model-only / NO BET on all nine** — no `totals_1st_1_innings` line was pulled at 11:00 (the priced sweep
is the 16:00 run's job, and a 1st-inning line before lineups post is not worth the credits). A read is
promoted to a bet only if a real posted line leaves it clearing the **+2pp** gate.

---
## Tonight's reads — 2026-08-19 (15-game slate; model-only at 11:00 — the priced `totals_1st_1_innings` sweep is the 16:00 run)

> ### ⚙ 8/18 CLOSED AT 8-7 (53%) — and the directional pattern this file has been tracking all month REVERSED
>
> `nrfi_settle.py --apply` stamped **all 15 rows** off `linescore.innings[0]` at session start, and the
> stamps were verified present before this section was written. **The YRFI leans went 7-3 while the NRFI
> leans went 1-4** — the exact inverse of the "NRFI side carries the board" pattern that had held on five
> of the previous six slates. Record moves **232-190 → 240-197 (55%)** (NRFI 120-96 · YRFI 120-101).
>
> ⛔ **THE −5pp YRFI CORRECTION IS RETIRED, at the completed review rather than after a bad day.** It was
> pre-registered on 8/6 to run ~10 slates; it has now run twelve. Across those slates it graded neutral
> (8/6, 8/7, 8/11), positive (8/8, 8/12) and negative (8/9, 8/10, 8/13, 8/15, 8/18) — i.e. **no measurable
> effect over its own pre-registered window**, which is the outcome that retires an experiment rather than
> extending it. ⚠ The specific reason it is retired rather than tapered is the one-way-ratchet concern
> logged on 8/7 and never resolved: **it only ever moves reads toward NRFI**, so on the two most lopsided
> slates in the window (8/9 and 8/18, both carried decisively by YRFI) it was pushing every flipped read
> onto the losing side. **Today's fifteen reads below are the RAW model with no correction applied.**
>
> ⚠ **Three of tonight's fifteen are explicitly LOW CONFIDENCE rather than modelled complete:** **ATH @ KC**
> is E4 on *both* halves (neither probable posted at 11:00), and **SF @ CLE** and **LAA @ HOU** each rest
> half their read on a pitcher with **zero 2026 MLB innings** (Matt Wilkinson, Ethan Pecko). ⚠ **Weather is
> PENDING at eleven of fifteen parks**; the four that posted (PNC, Citi, Target, Wrigley) are noted in-row.
> ⚠ **`wind_out_over` was RETIRED from the truep.py registry this run** (n=28, 46%, skill −0.0025, CLV
> coverage 0/28), so the three wind-out parks contribute **0.0pp** to their reads — a change that affects
> this file's park half as much as the main build's.
>
| Game | Lean | TrueP | Why |
|---|---|---|---|
| DET @ PIT (Jobe/Skenes) | **NRFI** | 56% | PNC is a pitcher park and **Skenes' 1.12 WHIP** owns the home half; the risk is all in Jobe's — 8.2 MLB IP all year at 1.38, and 6 ER in 3.2 IP on 8/14. Wind 9 mph OUT to LF at 77°F is a mild counter and, as of this run, contributes **0.0pp** — `wind_out_over` was retired from the registry today. |
| SD @ NYM (King/Stock) | **YRFI** | 60% | ⭐ **Strongest soft-half on the board: Robert Stock's 1.86 WHIP** over 12.1 IP / 3 GS is the worst among today's 30 probables, and San Diego's top three are the better trio. Citi Field's suppression is the only thing holding this under 62%. |
| ATL @ MIN (Smith-Shawver/Bradley) | **YRFI** | 57% | Both halves are soft: **Smith-Shawver has 4.1 IP all season and last pitched 7/29** (three weeks idle — likely a piggyback), and **Bradley's last three read 15 IP / 11 ER (6.60)** against a 3.98 season line. Two first-inning question marks in a park with wind out. |
| CWS @ CHC (Newcomb/Holmes) | **NRFI** | 58% | ⛔ **E5 — Chicago's White Sox half is a bullpen game**: Newcomb's last three outings are 1.0, 1.1 and 2.1 IP; he is a reliever listed as a probable. An opener's first inning is his *best* inning, and **Holmes (1.07) is clean**. **Wrigley wind 7 mph IN from LF at 72°F** supports it. |
| AZ @ BOS (Pfaadt/Tolle) | **NRFI** | 57% | The two cleanest WHIPs in one game — **Tolle 1.06 (and 8.0 IP / 0 ER on 8/13), Pfaadt 1.13**. Fenway is the counter-signal and it is a real one; weather is PENDING at 11:00 so the park read is geometry only. |
| MIA @ PHI (Alcantara/Nola) | **YRFI** | 55% | Citizens Bank plus Philadelphia's top three against **Alcantara's 6.86 K/9** — he limits damage but does not miss bats. ⚠ Nola's 1.45 season WHIP overstates his current form (**last three: 16 IP / 6 ER, 22 K**), which is why this is 55% and not 59%. |
| NYY @ BAL (Warren/Bassitt) | **YRFI** | 60% | Two of the worst WHIPs on the board in a hitter park — **Bassitt 1.62 and freshly back from a two-month absence** (one outing since 6/3), **Warren 1.41 with 8 ER in his last 16 IP** — against the Yankees' top three. Camden does the rest. |
| SF @ CLE (Wilkinson/Messick) | **NRFI** | 55% | ⚠ **LOW CONFIDENCE — half this read is unpriceable.** Matt Wilkinson has **zero 2026 MLB innings**; a debut arm's first inning is the widest distribution on the board. The other half is strong: **Messick 1.03 WHIP, 2.13 ERA over his last five**, against the league's weakest offence (51-74, −74). |
| STL @ CIN (Liberatore/Burns) | **YRFI** | 57% | GABP plus **Liberatore's 1.48** as the soft half, against a St. Louis club that is 7-3 in its last ten. **Burns (1.09) is the clean half** and is the reason this is not 60%+. |
| TOR @ TB (Scherzer/Rasmussen) | **NRFI** | 58% | ⭐ **Rasmussen's 0.91 WHIP is the best on the board** and his last four starts allowed 2, 0, 0 and 2 ER. ⚠ Scherzer's 1.46 season WHIP is stale-high — **his last three are 16.1 IP / 4 ER (2.20)** — which strengthens this read rather than weakening it. Dome, no weather input. |
| ATH @ KC (TBD/TBD) | **NRFI** | 51% | ⛔ **E4 on BOTH halves — neither probable is posted at 11:00.** Written at a near-coin-flip with a slight Kauffman lean and **flagged as the least-informed read on the board**; the 16:00 run re-reads it once the names post. Do not treat 51% as a modelled number. |
| SEA @ MIL (Gilbert/May) | **NRFI** | 59% | Two arms in form: **Gilbert 0.99 WHIP, 19.2 IP / 5 ER over his last three**, and **May 18 IP / 3 ER (1.50) over his**. Dome, no weather input. ⚠ The counter is that Milwaukee scored 22 runs in this park yesterday, but a 22-run game is a poor predictor of one inning. |
| WSH @ TEX (Cavalli/Rocker) | **YRFI** | 56% | **Rocker is the soft half and it is not close: 13.1 IP / 12 ER (8.10) over his last three**, including 6 ER in 3.0 IP on 8/9. Cavalli is the opposite (**20 IP / 5 ER, with 8.0 IP / 0 ER / 10 K on 8/13**), so the whole read rests on Texas's half of the inning. |
| LAA @ HOU (Ureña/Pecko) | **YRFI** | 55% | ⚠ **LOW CONFIDENCE — Ethan Pecko has zero 2026 MLB innings**, so Houston's half is a debut and unpriceable. Ureña (2.67 / 1.25) is a genuinely good half, which is what keeps this at 55% rather than 60%. |
| LAD @ COL (Sasaki/Freeland) | **YRFI** | 64% | ⭐ **Board's strongest lean. Coors, plus Freeland at 6.27 / 1.44 as the soft half, plus the Dodgers' top three** — the same shape that has been this tracker's most reliable YRFI profile all season. Weather PENDING at 11:00; the 16:00 run owns the wind re-read. |

**Model-only / NO BET on all fifteen** — no `totals_1st_1_innings` line was pulled at 11:00 (the priced
sweep is the 16:00 run's job, and a 1st-inning line before lineups post is not worth the credits). A read
is promoted to a bet only if a real posted line leaves it clearing the **+2pp** gate.
...
>
> ⭐ **8/17 WAS THE TRACKER'S BEST DAY ON RECORD: 9-2 (82%).** Both misses are informative rather
> than random. **AZ @ BOS (YRFI 57%)** was flagged **E4** at the time with Boston's probable unlisted,
> i.e. half the read was admittedly unresolved — and it went 0-0. **LAD @ COL (YRFI 64%)** was the
> board's strongest lean and it is the one that got **priced for the first time ever**: the market
> said 55.1% no-vig, we anchored to the market, wrote **TrueP 56.6%**, called it under the gate and
> **NO BET** — and the first inning went **0-0**. ⭐ **The tracker's own debut rule beat its own model
> number on the first live test.** Market-anchoring stays mandatory; a bare model lean is not a price.
>
> ⚠ **Method note carried forward:** yesterday's reads leaned on WHIP and top-of-order quality over
> season ERA where the two disagree, and that produced 9-2. Same weighting applied today. **Weather is
> PENDING at all fifteen parks at 11:00**, so every park half of every read below is dome/geometry only
> — no wind input anywhere, which is the same constraint that closed the main build's totals market.

| Game | SP matchup (ERA/WHIP) | Lean | TrueP | Driver |
|---|---|---|---|---|
| NYY @ BAL | Rodón 3.30/1.25 · **Baz 4.00/1.36** | **YRFI** | 56% | Camden is a genuine hitter's park and Baz's 1.36 is the softer half against a Yankee 1-2-3 that is the better of the two. |
| DET @ PIT | **Montero 3.22/0.99** · Ashcraft 3.82/1.10 (9.87 K/9) | **NRFI** | 60% | ⭐ **Second-strongest NRFI.** Two sub-1.10 WHIPs at PNC — the cleanest combined first-inning traffic profile on the board. |
| SF @ CLE | **Whisenhunt 6.11/1.71** · Griffin 3.25/1.11 | **YRFI** | 55% | Whisenhunt's **1.71 is the worst WHIP among today's probables**; the only thing holding this down is that San Francisco's top-of-order (−67 diff) is the weakest half he faces. |
| STL @ CIN | Leahy 3.38/1.34 · **Abbott 4.13/1.42** | **YRFI** | 59% | Great American again — the NL's best hitter's park — with **both** arms over 1.30 WHIP. Traffic on both halves is the read, not either ERA. |
| TOR @ TB | Soriano 3.16/1.25 · **Martinez 2.74/1.09** | **NRFI** | 57% | Two quality arms under a fixed roof, so no weather offset exists in either direction; Martinez's 1.09 is the backbone. |
| MIA @ PHI | **Gibson 5.61/1.54 (0 GS — BULLPEN GAME)** · **Wheeler 2.89/1.00, 10.69 K/9** | **NRFI** | 54% | ⚠ **A genuinely split read.** Wheeler is the single strongest first-inning suppressant on the slate (1.00 WHIP, board-best K/9); Gibson is an opener with a 1.54 WHIP facing the Phillies' 1-2-3 at CBP. Wheeler's half carries it, but only just — hence 54% and not 60%. |
| AZ @ BOS | **Kelly 5.11/1.48** · Suarez 3.25/1.19 | **YRFI** | 58% | Kelly's 1.48 at Fenway is first-inning traffic in the park least forgiving of it; his last three starts allowed 5, 1 and 6 ER. |
| SD @ NYM | Ray 3.28/1.33 · **Thornton 2.78/1.10** | **NRFI** | 54% | Citi Field suppresses and Thornton's 1.10 is real, but Ray's 1.33 with a 7.72 K/9 puts men on — this is the least confident NRFI on the board. |
| ATH @ KC | **Perkins 7.27/1.52** · **KC = TBD** | **YRFI** | 58% | Perkins carries the **worst ERA on the slate at 7.27** with a 1.52 WHIP. ⛔ **E4 — Kansas City's probable is unlisted at 11:00**, so half this read is unresolved exactly as AZ@BOS was yesterday (and that one lost). |
| ATL @ MIN | Mahle 4.64/1.32 · **Matthews 5.34/1.35** | **YRFI** | 58% | Two soft WHIPs facing each other and Atlanta's 1-2-3 is among the best on the board; Target Field is neutral enough not to offset it. |
| **SEA @ MIL** | **Miller 3.39/0.99** · **Harrison 2.99/1.13, 10.99 K/9** | **NRFI** | **61%** | ⭐ **Board's strongest read of either kind.** Miller's 0.99 WHIP against Harrison's 10.99 K/9 — the best combined arms on the slate — under a roof that removes every weather offset. |
| WSH @ TEX | **Kent 6.75/2.00 (1 GS, 4.0 IP)** · Quantrill 3.44/1.20 | **YRFI** | 57% | ⚠ **C3 — a 2.00 WHIP on FOUR major-league innings is a direction, not a sample**, and the read is shaded down for it. Roof removes the wind half. |
| CWS @ CHC | **CWS = TBD** · Gausman 4.53/1.27 (8.94 K/9) | **YRFI** | 55% | ⛔ **E4 — Chicago's away probable is unlisted**, and Wrigley's wind is PENDING, so **both** halves of the park×arm read are unresolved. Lowest-confidence row on the board. |
| **LAA @ HOU** | **Klassen 5.52/2.11 (4 GS, 14.2 IP)** · **Javier 6.68/1.70** | **YRFI** | **62%** | ⭐ **Board's strongest YRFI: two of the three worst WHIPs on the slate facing each other.** Javier's 6.68 is a post-return line over 6 GS; Klassen's 2.11 is a 14-inning rookie sample (C3). Roof, so no weather offset either way. |
| LAD @ COL | Lauer 4.67/1.24 (**6.69 over the LAD stint**) · **Feltner 5.59/1.49** | **YRFI** | 60% | Coors plus the best 1-2-3 on the slate. ⚠ **Shaded DOWN from the 64% this same read carried yesterday — because yesterday it LOST and the market's 55.1% beat it.** 60% is the honest number after that lesson, not 64%. |

**5 NRFI · 10 YRFI.** All model-only at 11:00; none is a bet unless a real `totals_1st_1_innings`
line clears the +2pp gate at the 16:00 sweep. ⚠ **Three rows (ATH@KC, CWS@CHC, and the Gibson half of
MIA@PHI) rest on an unlisted or non-starting arm** — yesterday the one E4-gated read on the board was
one of the day's only two losses, so those three are explicitly the least trustworthy of the fifteen.

---

## Tonight's reads — 2026-08-17 (11-game slate incl. a STL@CIN doubleheader; model-only at 11:00 — the priced `totals_1st_1_innings` sweep is the 16:00 run)

> **✅ 8/16 AUTO-SETTLED at session start** — 6 rows stamped off `linescore.innings[0]` by
> `nrfi_settle.py --apply`. **Record now 223-188 (54%)** (NRFI 117-92 · YRFI 106-96).
>
> ⚠ **Calibration note carried in from yesterday's review: the KC @ LAA YRFI read (59%, leaned entirely
> on Ryan Johnson's 6.71 ERA) produced a THREE-run game with a clean first.** It was the day's worst
> single read, and the lesson is method-level: **a bad season ERA is a weak proxy for first-inning run
> risk.** Today's reads lean on WHIP and top-of-order quality ahead of ERA where the two disagree.
>
> ⚠ **Structural note: game 1 of the Cincinnati doubleheader (13:41 ET) is the only game that starts
> before the 16:00 run**, so it is model-only permanently. The other ten are priceable at 16:00.

| Game | SP matchup (ERA/WHIP) | Lean | TrueP | Driver |
|---|---|---|---|---|
| **STL @ CIN (G1)** ✅ **W** | Mathews 3.60/1.80 (**1 GS**) · **Emanuel = BULLPEN GAME (0 career GS)** | **YRFI** | 62% | ⭐ Board's second-strongest: two arms with no starting workload between them at the NL's best hitter's park, 81°F with 5 mph out to LF. ⛔ E5 — no ML/total leg built here. |
| STL @ CIN (G2) | Pallante 3.46/1.22 · **Lowder 5.15/1.48** | **YRFI** | 58% | GABP again; Lowder's 1.48 WHIP is first-inning traffic. ⚠ Counter: his last three starts are 2 ER apiece. |
| BAL @ TB | Young 3.33/1.32 · **McClanahan 3.09/1.12, 8.27 K/9** | **NRFI** | 54% | McClanahan is a genuine NRFI backbone and the Trop dome removes every weather offset; Young's 1.32 is the softer half. |
| MIA @ PHI | Junk 4.41/1.32 · **Sánchez 2.54/1.19, 10.46 K/9** | **NRFI** | 55% | ⭐ Strongest NRFI: the best arm on the board by ERA and K/9, against a Miami top-of-order that is the weaker of the two. |
| DET @ PIT | Valdez 4.26/1.39 · Mlodzinski 3.79/1.40 | **YRFI** | 55% | Two 1.4 WHIPs put men on early; PNC being a pitcher's park is the only thing holding this off 58%. |
| AZ @ BOS | Bratt 3.74/1.40 (**7 GS, 33.2 IP**) · **BOS = TBD** | **YRFI** | 57% | Fenway plus a 1.40 WHIP on a 33-inning sample. ⛔ **E4 — Boston's probable is unlisted at 11:00**, so half this read is unresolved. |
| SD @ NYM | **Buehler 4.88/1.43** · McLean 3.42/1.13, 10.47 K/9 | **YRFI** | 55% | Buehler's 1.43 is the clearest first-inning flag; McLean and a pitcher park are why this is not higher. |
| ATH @ KC | **Barnett 6.16/1.45** · Wacha 3.46/1.15 | **YRFI** | 57% | Barnett is the worst line among today's probables and a converted reliever averaging 4.2 IP; Kauffman's size caps it. |
| ATL @ MIN | Pérez 2.96/1.15 · **Ober 4.64/1.23** | **YRFI** | 55% | Atlanta's 1-2-3 is the driver, not Ober's ERA — the read is the loaded top of order against the weaker arm. |
...
YRFI leans rest on park and workload rather than ERA.** **All reads are model-only; no
`totals_1st_1_innings` line has been pulled at 11:00 and none is bet.**

---

## Tonight's reads — 2026-08-16 (15-game slate; model-only at 11:00 — the priced `totals_1st_1_innings` sweep is the 16:00 run)

> **✅ 8/15 AUTO-SETTLED at session start** — 12 rows stamped off `linescore.innings[0]` by
> `nrfi_settle.py --apply`. **Record now 215-181 (54%)** (NRFI 113-91 · YRFI 102-90).
>
> ⚠ **Structural note for today: NINE of fifteen first pitches land before the 16:00 run** (earliest
> 12:15 ET). Those nine are model-only permanently — there is no later run that can price them on a
> real `totals_1st_1_innings` line, so they are logged as leans and never as bets.

| Game | SP matchup (ERA/WHIP) | Lean | TrueP | Driver |
|---|---|---|---|---|
| BAL @ TB | Rogers 4.21/1.25 · **F. Peralta 5.33/1.50** | **YRFI** | 58% | Peralta's 1.50 WHIP is the board's worst — 1st-inning traffic is the input, and the Trop dome removes any weather offset. |
| BOS @ PIT | Sandoval 3.30/**1.70** · **Bachar = BULLPEN GAME** | **YRFI** | 58% | A 1.70 WHIP puts men on immediately; PNC has 10 mph out to CF. ⛔ Pittsburgh's "probable" is an opener (E5). |
| AZ @ ATL | Soroka 2.92/1.10 (**post-IL, 1 start back**) · Elder 4.03/1.25 | **YRFI** | 56% | Atlanta's 1-2-3 vs a 4.1-IP ramp arm; 92°F and 7 mph out to LF. |
| NYY @ TOR | Weathers 3.69/1.21 · **Cease 2.40/1.07, 13.09 K/9** | **NRFI** | 52% | Cease is a genuine NRFI backbone; the Yankee top-of-order is the counter and it is why this is 52% and not 58%. |
| WSH @ NYM | **Irvin 5.79/1.32** · Scott 3.45/1.30 | **YRFI** | 60% | Irvin is the single clearest 1st-inning flag on the slate; Citi has the board's strongest wind at 13 mph out to LF. |
| SD @ CLE | Mize 3.38/1.09 · Bibee 3.99/1.13 | **NRFI** | 51% | Two competent arms in a pitcher park; the 9 mph out to RF is what keeps this off a stronger lean. |
| CWS @ DET | Burke 2.99/1.08 · **Anderson (swingman, 3.91/1.30)** | **NRFI** | 51% | Burke's 1.08 WHIP carries it; Anderson has been stretched to 3.2 and 4.0 IP but opens sharp. |
| MIA @ CIN | Pérez 3.39/1.11 · **Lodolo 4.86/1.46** | **YRFI** | 61% | ⭐ Strongest YRFI: the NL's best hitter's park, 87°F, 9 mph out to LF, against a 1.46 WHIP. |
| PHI @ MIN | **Painter 6.27/1.53** · **Kremer 5.25/1.17** | **YRFI** | 62% | ⭐ Board's highest: the worst starting pair on the slate by ERA, both over 5.00, with 10 mph out to RF. |
| STL @ CHC | Dobbins 3.40/1.31 · **Cabrera 5.10/1.40** | **YRFI** | 57% | Cabrera over 5.00 at Wrigley; ⚠ wind PENDING, so the park half of the read is unresolved. |
| COL @ SF | **Hughes 5.61/1.28** · Tidwell 2.78/1.10 (2 GS) | **YRFI** | 54% | Hughes is bad enough to lean Over on his own; Oracle Park is the reason this sits at 54 and not 60. |
| TEX @ ATH | Bradford 2.38/1.15 (**2 GS, 11.1 IP**) · **Lopez 5.38/1.56** | **YRFI** | 58% | Lopez's 1.56 WHIP in a small hot park; ⚠ Bradford's sample is 11.1 IP — the low ERA is not load-bearing. |
| KC @ LAA | Cameron 4.45/1.29 · **Johnson 6.71/1.56** | **YRFI** | 59% | Johnson is the worst ERA among today's probables and pairs a 1.56 WHIP with it. |
| MIL @ LAD | **Henderson 2.88/0.91** · **Skubal 2.93/0.93, 10.70 K/9** | **NRFI** | 57% | ⭐ Strongest NRFI: the best pitching matchup on the board by some distance — two sub-0.95 WHIPs. |
| SEA @ HOU | Woo 4.15/1.10 · Brown 3.68/1.27 | **NRFI** | 52% | Woo's 1.10 WHIP in a dome; Brown's 1.27 is the softer half and caps the conviction. |

**Slate shape: 5 NRFI / 10 YRFI.** The YRFI tilt is not a house view — it is that **six of today's fifteen
probables carry an ERA over 5.00** (F. Peralta 5.33, Irvin 5.79, Lodolo 4.86, Painter 6.27, Kremer 5.25,
Cabrera 5.10, Hughes 5.61, Lopez 5.38, Johnson 6.71 — nine, in fact), which is the most run-prone set of
starters the tracker has logged. **All reads are model-only; no `totals_1st_1_innings` line has been
pulled at 11:00 and none is bet.**
...
|------|---------|------|-------|-------|--------|----------------------------|
| 8/25 | KC @ TOR (Lugo/Scherzer) | YRFI | 62% | _model-only_ | **W** (1st 1-1 → YRFI) | Both arms in disaster form: Lugo 4.71/1.38 (5 ER last start), Scherzer 7.40/1.51 (7 ER last start). Worst combined line on the board — two bad starters = YRFI regardless of park. |
| 8/25 | MIL @ NYM (Harrison/opp) | NRFI | 61% | _model-only_ | **W** (1st 0-0 → NRFI) | Harrison 2.84/1.11 K/9 11.17 — ace NRFI arm in dominant form (5 IP / 0 ER / 8K last start 8/18). NYM arm TBD; read rests on Harrison dominating his half. |
| 8/25 | TEX @ CWS (deGrom/CWS arm) | NRFI | 60% | _model-only_ | **L** (1st 0-6 → YRFI) | deGrom 3.77/1.13 / K/9 11.16 — last start 6 IP / 0 ER / 10K. Elite control arm leans NRFI even in Rate Field. ⚠ CWS arm TBD; one-sided read. |
| 8/25 | PHI @ SEA (Nola/Kirby) | YRFI | 56% | _model-only_ | **L** (1st 0-0 → NRFI) | Nola 5.12 ERA worst SP on Phi staff; Kirby "disaster last start 3.1/6 ER" (from build). Two mediocre arms in a pitcher-friendly park — park fights it. LOW CONF. |
...
| 8/24 | TEX @ CWS (Rocker/Urquidy) | YRFI | 58% | _model-only_ | **W** (1st 2-0 → YRFI) | Urquidy 16-IP fresh sample, Rocker 1.40 WHIP; Rate Field hitter park. |
| 8/24 | COL @ WSH (Feltner/Cavalli) | YRFI | 57% | _model-only_ | **L** (1st 0-0 → NRFI) | Feltner 5.97/1.53 worst real-SP line; Cavalli 3.22 caps it at 57%. |
| 8/24 | CLE @ LAA (Messick/Klassen) | YRFI | 56% | _model-only_ | **L** (1st 0-0 → NRFI) | Messick elite but Klassen (rookie) 1.62 WHIP leaks early — one-sided. |
| 8/24 | TB @ DET (Rasmussen/Valdez) | NRFI | 57% | _model-only_ | **L** (1st 2-0 → YRFI) | Rasmussen 0.95 best WHIP, Valdez GB arm; spacious Comerica. |
| 8/24 | PIT @ SD (Ashcraft/R.Ray) | NRFI | 57% | _model-only_ | **W** (1st 0-0 → NRFI) | Petco 2nd-most suppressing; PIT top weak; both arms solid. |
| 8/24 | BOS @ MIA (Suarez/Alcantara) | NRFI | 56% | _model-only_ | **W** (1st 0-0 → NRFI) | Two steady arms (3.41/3.46), loanDepot dome; BOS top counterweight. |
| 8/24 | CIN @ SF (Burns/TBD) | NRFI | 55% | _model-only_ | **L** (1st 0-2 → YRFI) | E4 — SF probable TBD; rests on Burns 2.51/1.11 + Oracle. LOW CONF. |
...
| 8/23 | ATL @ MIL (Mahle/Drohan) | NRFI | 56% | _model-only_ | **W** (1st 0-0 → NRFI) | **Mahle's 4. |
...
| 8/15 | MIL @ LAD (Misiorowski/Wrobleski) | NRFI | 61% | **U0.5 −136 BetRivers → no-vig 52.9%** | **W** (1st 0-0 → NRFI) | 0.74 WHIP — lowest on the board; baserunners are the input. ⚠ **16:00: PRICED. Raw gap +8.1pp — DECLINED**, not promoted: model number is unanchored to market, `market_disagrees` fires, 8.9% hold. See the 16:00 block above. |
| 8/15 | AZ @ ATL (E-Rodriguez/Holmes) | NRFI | 57% | _model-only_ | **L** (1st 0-2 → YRFI) | Two sub-3.50 ERA arms, no park/air help. |
| 8/15 | SEA @ HOU (Hancock/Wesneski) | NRFI | 56% | _model-only_ | **L** (1st 0-1 → YRFI) | Dome, neutral air, two 1.1x WHIPs. |
...
| 8/15 | BAL @ TB (Bradish/Seymour) | NRFI | 54% | _model-only_ | **W** (1st 0-0 → NRFI) | Seymour 10.56 K/9 in a dome; Bradish 1.34 the drag. |
...
| 8/15 | KC @ LAA (Dobnak/Detmers) | NRFI | 54% | _model-only_ | **W** (1st 0-0 → NRFI) | Detmers 10.40 K/9 vs a KC offence at −113 run diff. |
| 8/15 | PHI @ MIN (Luzardo/Prielipp) | NRFI | 53% | _model-only_ | **W** (1st 0-0 → NRFI) | Luzardo 11.03 K/9 carries it; Prielipp vs the PHI top is the risk. |
| 8/15 | BOS @ PIT (Gray/Jones) | NRFI | 53% | _model-only_ | **W** (1st 0-0 → NRFI) | PNC suppresses; Jones gave 8 ER in 3.0 IP last out — the live YRFI half. |
| 8/15 | COL @ SF (Lorenzen/Webb) | YRFI | 53% | _model-only_ | **L** (1st 0-0 → NRFI) | Lorenzen 1.88 WHIP is a 1st-inning traffic magnet; Oracle is the counter. |
| 8/15 | SD @ CLE (TBA/Cantillo) | YRFI | 54% | _model-only_ | **W** (1st 1-1 → YRFI) | Cantillo 1.48 WHIP; E4 LOW CONFIDENCE — half the input missing. |
| 8/15 | TEX @ ATH (Gore/Ginn) | YRFI | 54% | _model-only_ | **L** (1st 0-0 → NRFI) | Sutter Health hot small-park; both arms mid. |
| 8/15 | WSH @ NYM (Lord-BULLPEN/Manaea) | YRFI | 55% | _model-only_ | **W** (1st 0-2 → YRFI) | Manaea 1.32 is the real input; Lord's 1-2 innings are fresh, not soft. |
| 8/15 | MIA @ CIN (Gusto/Singer) | YRFI | 58% | _model-only_ | **L** (1st 0-0 → NRFI) | Weakest pair on the board in the NL's best hitter's park. |

| 8/16 | BAL @ TB (Rogers/F.Peralta) | YRFI | 58% | _model-only_ | **L** (1st 0-0 → NRFI) | Peralta 1.50 WHIP is the board's worst; Trop dome removes any weather offset. |
| 8/16 | BOS @ PIT (Sandoval/Bachar-BULLPEN) | YRFI | 58% | _model-only_ | **W** (1st 0-2 → YRFI) | Sandoval 1.70 WHIP puts men on early; PNC 10mph out to CF. E5 on the PIT side. |
| 8/16 | AZ @ ATL (Soroka/Elder) | YRFI | 56% | _model-only_ | **L** (1st 0-0 → NRFI) | ATL top vs a post-IL 4.1-IP ramp arm; 92°F, 7mph out to LF. |
| 8/16 | NYY @ TOR (Weathers/Cease) | NRFI | 52% | _model-only_ | **W** (1st 0-0 → NRFI) | Cease 2.40/1.07, 13.09 K9 is a real NRFI arm; NYY's top-of-order is the counter. |
| 8/16 | WSH @ NYM (Irvin/Scott) | YRFI | 60% | _model-only_ | **L** (1st 0-0 → NRFI) | Irvin 5.79 is the clearest 1st-inning flag; Citi 13mph out to LF, strongest wind on the board. |
| 8/16 | SD @ CLE (Mize/Bibee) | NRFI | 51% | _model-only_ | **W** (1st 0-0 → NRFI) | Two competent arms in a pitcher park; 9mph out to RF caps the lean. ⚠ **16:00: BOTH lineups CONFIRMED, but the game is RAIN-DELAYED** (first pitch pushed 13:40 → 16:51 ET). Read held at 51% — a delay adds start-length noise on both arms, which cuts toward NRFI marginally but not enough to move a 51% model-only lean. ⚠ Mize's 3.38 is a blend hiding **9.64/2.04 over 9.1 IP post-trade**. |
| 8/16 | CWS @ DET (Burke/Anderson) | NRFI | 51% | _model-only_ | **W** (1st 0-0 → NRFI) | Burke 1.08 WHIP carries it; Anderson is a stretching-out swingman but opens sharp. |
| 8/16 | MIA @ CIN (Pérez/Lodolo) | YRFI | 61% | _model-only_ | **L** (1st 0-0 → NRFI) | GABP + 87°F + 9mph out to LF vs Lodolo's 1.46 WHIP. |
| 8/16 | PHI @ MIN (Painter/Kremer) | YRFI | 62% | _model-only_ | **W** (1st 0-4 → YRFI) | Worst SP pair on the slate (6.27 and 5.25); Target Field 10mph out to RF. |
| 8/16 | STL @ CHC (Dobbins/Cabrera) | YRFI | 57% | _model-only_ | **W** (1st 2-0 → YRFI) | Cabrera 5.10 at Wrigley; ⚠ wind PENDING so the park half is unresolved. |
| 8/16 | COL @ SF (Hughes/Tidwell) | YRFI | 54% | _model-only_ | **W** (1st 0-1 → YRFI) | Hughes 5.61 leans Over alone; Oracle Park is why this is 54 not 60. |
| 8/16 | TEX @ ATH (Bradford/Lopez) | YRFI | 58% | _model-only_ | **L** (1st 0-0 → NRFI) | Lopez 1.56 WHIP in a small hot park; ⚠ Bradford's 11.1 IP sample is not load-bearing. |
...
| 8/17 | STL @ CIN G2 (Pallante/Lowder) | YRFI | 58% | _model-only_ | **W** (1st 0-1 → YRFI) | Lowder 5.15/1.48 at GABP. ⚠ Counter: his last three starts are 2 ER apiece. |
| 8/17 | BAL @ TB (Young/McClanahan) | NRFI | 54% | _model-only_ | **W** (1st 0-0 → NRFI) | McClanahan 3.09/1.12 is a real NRFI arm; Trop dome removes weather. Young's 1.32 is the softer half. |
| 8/17 | MIA @ PHI (Junk/Sánchez) | NRFI | 55% | _model-only_ | **W** (1st 0-0 → NRFI) | ⭐ Strongest NRFI: Sánchez 2.54/1.19 with 10.46 K/9 vs the weaker top-of-order. |
| 8/17 | DET @ PIT (Valdez/Mlodzinski) | YRFI | 55% | _model-only_ | **W** (1st 2-0 → YRFI) | Two 1.4 WHIPs; PNC being a pitcher park is the only thing holding this off 58%. |
| 8/17 | AZ @ BOS (Bratt/TBD) | YRFI | 57% | _model-only_ | **L** (1st 0-0 → NRFI) | Fenway + a 1.40 WHIP on 33.2 IP. ⛔ E4 — Boston's probable unlisted at 11:00, half the read unresolved. |
| 8/17 | SD @ NYM (Buehler/McLean) | YRFI | 55% | _model-only_ | **W** (1st 0-2 → YRFI) | Buehler 1.43 WHIP is the flag; McLean 10.47 K/9 and a pitcher park cap it. |
| 8/17 | ATH @ KC (Barnett/Wacha) | YRFI | 57% | _model-only_ | **W** (1st 1-3 → YRFI) | Barnett 6.16/1.45, a converted reliever averaging 4.2 IP; Kauffman's size caps it. |
| 8/17 | ATL @ MIN (Pérez/Ober) | YRFI | 55% | _model-only_ | **W** (1st 0-4 → YRFI) | Driver is Atlanta's 1-2-3, not Ober's ERA — loaded top vs the weaker arm. |
| 8/17 | CWS @ CHC (Castillo/Imanaga) | YRFI | 56% | _model-only_ | **W** (1st 0-2 → YRFI) | Castillo 4.96/1.35 at Wrigley. ⚠ Wind PENDING, park half unresolved. |
| 8/17 | LAD @ COL (Snell/Sugano) | YRFI | 64% | _model-only_ | **L** (1st 0-0 → NRFI) | ⭐ Board's strongest: Coors auto-tilt + the best 1-2-3 on the slate + Sugano's 5.23 K/9 = contact from pitch one. |
| 8/18 | NYY @ BAL (Rodón/Baz) | YRFI | 56% | _model-only_ | **L** (1st 0-0 → NRFI) | Camden hitter park; Baz 1.36 WHIP is the softer half vs the better 1-2-3. |
| 8/18 | DET @ PIT (Montero/Ashcraft) | NRFI | 60% | _model-only_ | **W** (1st 0-0 → NRFI) | ⭐ Two sub-1.10 WHIPs (0.99 / 1.10) at PNC — cleanest combined first-inning profile on the board. |
| 8/18 | SF @ CLE (Whisenhunt/Griffin) | YRFI | 55% | _model-only_ | **W** (1st 0-3 → YRFI) | Whisenhunt's 1.71 is the worst WHIP among today's probables; held down only by SF's weak top-of-order. |
| 8/18 | STL @ CIN (Leahy/Abbott) | YRFI | 59% | _model-only_ | **L** (1st 0-0 → NRFI) | GABP with BOTH arms over 1.30 WHIP — traffic on both halves, not either ERA. |
| 8/18 | TOR @ TB (Soriano/Martinez) | NRFI | 57% | _model-only_ | **L** (1st 2-0 → YRFI) | Two quality arms under a fixed roof; Martinez 1.09 is the backbone. |
| 8/18 | MIA @ PHI (Gibson/Wheeler) | NRFI | 54% | _model-only_ | **L** (1st 0-1 → YRFI) | ⚠ Split read: Wheeler 1.00/10.69 K9 is the board's best suppressant, but Gibson is a 1.54-WHIP OPENER (0 GS) facing PHI's 1-2-3 at CBP. |
| 8/18 | AZ @ BOS (Kelly/Suarez) | YRFI | 58% | _model-only_ | **W** (1st 0-3 → YRFI) | Kelly 1.48 at Fenway; last three starts allowed 5, 1 and 6 ER. |
| 8/18 | SD @ NYM (Ray/Thornton) | NRFI | 54% | _model-only_ | **L** (1st 1-0 → YRFI) | Citi suppresses and Thornton is 1.10, but Ray's 1.33 puts men on — least confident NRFI on the board. |
| 8/18 | ATH @ KC (Perkins/TBD) | YRFI | 58% | _model-only_ | **W** (1st 1-0 → YRFI) | Perkins 7.27/1.52 is the worst ERA on the slate. ⛔ E4 — KC probable unlisted, half the read unresolved. |
| 8/18 | ATL @ MIN (Mahle/Matthews) | YRFI | 58% | _model-only_ | **W** (1st 0-2 → YRFI) | Two soft WHIPs (1.32 / 1.35) and Atlanta's 1-2-3 is among the slate's best; Target Field neutral. |
| 8/18 | SEA @ MIL (Miller/Harrison) | NRFI | 61% | _model-only_ | **L** (1st 0-1 → YRFI) | ⭐ Board's strongest read: Miller 0.99 WHIP vs Harrison 10.99 K/9, best combined arms, roof removes weather offset. |
| 8/18 | WSH @ TEX (Kent/Quantrill) | YRFI | 57% | _model-only_ | **W** (1st 0-1 → YRFI) | ⚠ C3 — Kent's 2.00 WHIP is on FOUR MLB innings; direction not sample. Roof removes the wind half. |
| 8/18 | CWS @ CHC (TBD/Gausman) | YRFI | 55% | _model-only_ | **W** (1st 0-2 → YRFI) | ⛔ E4 + wind PENDING — both halves of the park×arm read unresolved. Lowest-confidence row on the board. |
| 8/18 | LAA @ HOU (Klassen/Javier) | YRFI | 62% | _model-only_ | **L** (1st 0-0 → NRFI) | ⭐ Board's strongest YRFI: 2.11 and 1.70 WHIPs facing each other. Both C3 small samples; roof, no weather offset. |
| 8/18 | LAD @ COL (Lauer/Feltner) | YRFI | 60% | _model-only_ | **W** (1st 1-1 → YRFI) | Coors + best 1-2-3 on the slate. ⚠ Shaded DOWN from yesterday's 64% on this same read, which LOST to a 0-0 first — the market's 55.1% beat our model. |
| 8/19 | DET @ PIT (Jobe/Skenes) | NRFI | 56% | _model-only_ | **L** (1st 0-1 → YRFI) | PNC is a pitcher park and **Skenes' 1.12 WHIP** owns the home half; the risk is all in Jobe's — 8.2 MLB IP all year at 1.38, and 6 ER in 3.2 IP on 8/14. Wind 9 mph OUT to LF at 77°F is a mild counter and, as of this run, contributes **0.0pp** — `wind_out_over` was retired from the registry today. |
| 8/19 | SD @ NYM (King/Stock) | YRFI | 60% | _model-only_ | **L** (1st 0-0 → NRFI) | ⭐ **Strongest soft-half on the board: Robert Stock's 1.86 WHIP** over 12.1 IP / 3 GS is the worst among today's 30 probables, and San Diego's top three are the better trio. Citi Field's suppression is the only thing holding this under 62%. |
| 8/19 | ATL @ MIN (Smith-Shawver/Bradley) | YRFI | 57% | _model-only_ | **L** (1st 0-0 → NRFI) | Both halves are soft: **Smith-Shawver has 4.1 IP all season and last pitched 7/29** (three weeks idle — likely a piggyback), and **Bradley's last three read 15 IP / 11 ER (6.60)** against a 3.98 season line. Two first-inning question marks in a park with wind out. |
| 8/19 | CWS @ CHC (Newcomb/Holmes) | NRFI | 58% | _model-only_ | **W** (1st 0-0 → NRFI) | ⛔ **E5 — Chicago's White Sox half is a bullpen game**: Newcomb's last three outings are 1.0, 1.1 and 2.1 IP; he is a reliever listed as a probable. An opener's first inning is his *best* inning, and **Holmes (1.07) is clean**. **Wrigley wind 7 mph IN from LF at 72°F** supports it. |
| 8/19 | AZ @ BOS (Pfaadt/Tolle) | NRFI | 57% | _model-only_ | **L** (1st 0-1 → YRFI) | The two cleanest WHIPs in one game — **Tolle 1.06 (and 8.0 IP / 0 ER on 8/13), Pfaadt 1.13**. Fenway is the counter-signal and it is a real one; weather is PENDING at 11:00 so the park read is geometry only. |
| 8/19 | MIA @ PHI (Alcantara/Nola) | YRFI | 55% | _model-only_ | **L** (1st 0-0 → NRFI) | Citizens Bank plus Philadelphia's top three against **Alcantara's 6.86 K/9** — he limits damage but does not miss bats. ⚠ Nola's 1.45 season WHIP overstates his current form (**last three: 16 IP / 6 ER, 22 K**), which is why this is 55% and not 59%. |
| 8/19 | NYY @ BAL (Warren/Bassitt) | YRFI | 60% | _model-only_ | **W** (1st 0-1 → YRFI) | Two of the worst WHIPs on the board in a hitter park — **Bassitt 1.62 and freshly back from a two-month absence** (one outing since 6/3), **Warren 1.41 with 8 ER in his last 16 IP** — against the Yankees' top three. Camden does the rest. |
| 8/19 | SF @ CLE (Wilkinson/Messick) | NRFI | 55% | _model-only_ | **W** (1st 0-0 → NRFI) | ⚠ **LOW CONFIDENCE — half this read is unpriceable.** Matt Wilkinson has **zero 2026 MLB innings**; a debut arm's first inning is the widest distribution on the board. The other half is strong: **Messick 1.03 WHIP, 2.13 ERA over his last five**, against the league's weakest offence (51-74, −74). |
| 8/19 | STL @ CIN (Liberatore/Burns) | YRFI | 57% | _model-only_ | **L** (1st 0-0 → NRFI) | GABP plus **Liberatore's 1.48** as the soft half, against a St. Louis club that is 7-3 in its last ten. **Burns (1.09) is the clean half** and is the reason this is not 60%+. |
| 8/19 | TOR @ TB (Scherzer/Rasmussen) | NRFI | 58% | _model-only_ | **L** (1st 0-2 → YRFI) | ⭐ **Rasmussen's 0.91 WHIP is the best on the board** and his last four starts allowed 2, 0, 0 and 2 ER. ⚠ Scherzer's 1.46 season WHIP is stale-high — **his last three are 16.1 IP / 4 ER (2.20)** — which strengthens this read rather than weakening it. Dome, no weather input. |
| 8/19 | ATH @ KC (Springs/Lugo) | **YRFI** | 58% | _model-only_ | TBD | ⬆ **SUPERSEDES the 11:00 E4 row — the read FLIPS from a 51% NRFI coin-flip to a 58% YRFI lean now that both probables have posted.** At 11:00 neither name was listed and the row was written as the least-informed read on the board. `recheck.py` caught both at the 16:00 run: **Jeffrey Springs (ATH) 6.17 ERA / 1.48 WHIP over 22 GS** and **Seth Lugo (KC) 4.59 / 1.38 over 25 GS — but 15.2 IP / 13 ER = 7.47 over his last three.** **That is the worst combined first-inning matchup on the fifteen-game board**, and it is why the lean inverts rather than merely firming up. ⚠ **Springs has not started since 7/31 (19 days), so C3 pitch-count uncertainty applies to his half** — which cuts toward YRFI in the first inning specifically, since a rusty arm's command tends to be worst before he settles. ⚠ **Kauffman is a large park and wind is 6 mph IN FROM CF at 81°F, both of which argue the other way** — the park half of the read is genuinely against the pitching half, and 58% reflects that tension rather than ignoring it. ⚠ **NOT BET: no priced `totals_1st_1_innings` market was pulled for this game** (the sweep was confined to the three signal games), so this stays model-only at 0.0pp. |
| 8/19 | SEA @ MIL (Gilbert/May) | NRFI | 59% | _model-only_ | **L** (1st 4-0 → YRFI) | Two arms in form: **Gilbert 0.99 WHIP, 19.2 IP / 5 ER over his last three**, and **May 18 IP / 3 ER (1.50) over his**. Dome, no weather input. ⚠ The counter is that Milwaukee scored 22 runs in this park yesterday, but a 22-run game is a poor predictor of one inning. |
| 8/19 | WSH @ TEX (Cavalli/Rocker) | YRFI | 56% | _model-only_ | **L** (1st 0-0 → NRFI) | **Rocker is the soft half and it is not close: 13.1 IP / 12 ER (8.10) over his last three**, including 6 ER in 3.0 IP on 8/9. Cavalli is the opposite (**20 IP / 5 ER, with 8.0 IP / 0 ER / 10 K on 8/13**), so the whole read rests on Texas's half of the inning. |
| 8/19 | LAA @ HOU (Ureña/Pecko) | YRFI | 55% | _model-only_ | **L** (1st 0-0 → NRFI) | ⚠ **LOW CONFIDENCE — Ethan Pecko has zero 2026 MLB innings**, so Houston's half is a debut and unpriceable. Ureña (2.67 / 1.25) is a genuinely good half, which is what keeps this at 55% rather than 60%. |
| 8/19 | LAD @ COL (Sasaki/Freeland) | YRFI | 64% | _model-only_ | **L** (1st 0-0 → NRFI) | ⭐ **Board's strongest lean. Coors, plus Freeland at 6.27 / 1.44 as the soft half, plus the Dodgers' top three** — the same shape that has been this tracker's most reliable YRFI profile all season. Weather PENDING at 11:00; the 16:00 run owns the wind re-read. |
| 8/20 | STL @ CIN (McGreevy/Singer) | YRFI | 55% | _model-only_ | **W** (1st 1-0 → YRFI) | GABP is a genuine YRFI park; Singer 4.66 / 1.44 last three at 4.50 supports the soft-half read, though his last three are 6/3, 6/3, 6/3 — remarkably steady. McGreevy 3.48 / 1.22 with 6.20 K/9 limits damage but does not miss bats. Neither top-of-order elite. |
| 8/20 | SF @ CLE (Roupp/Williams) | NRFI | 58% | _model-only_ | **L** (1st 0-3 → YRFI) | ⭐ Williams' 11.48 K/9 over 149 IP owns the home half — whiffs are the cleanest 1st-inning suppressant. Progressive Field suppresses; wind 7 mph IN From RF at 77°F reinforcing. SF are 51-75, −74 differential — the worst offence on the board. Roupp 1.30 WHIP is the drag. |
| 8/20 | TOR @ TB (Bieber/Seymour) | NRFI | 57% | _model-only_ | **W** (1st 0-0 → NRFI) | Dome, no weather input. Seymour 10.74 K/9 owns the home half; last three 6/2, 6/1, 6/3. Bieber 4.99 / 1.56 season is stale-high — his last three at 3.64 refutes the aggregate. Neither top-of-order elite. |
| 8/20 | ATH @ KC (Jump/Dobnak) | YRFI | 55% | _model-only_ | **W** (1st 0-1 → YRFI) | Jump 4.50 / 1.45 with 6/2 recent variance (3.2/6, 6/2, 5.1/3) is the soft half. Dobnak 1.73 season on 6-GS small sample; 1.30 WHIP concedes traffic and K/9 5.40 is 2nd-lowest on the board. Kauffman neutral, wind 5 mph IN From CF at 79°F a mild counter. |
| 8/20 | ATL @ CWS (Holmes/Kay) | NRFI | 55% | _model-only_ | **W** (1st 0-0 → NRFI) | Two average-ish arms with steady recent form: Holmes 3.82 / 1.35 (last three 3.45), Kay 4.01 / 1.34 (last three 2.55). Rate Field neutral, wind 8 mph L To R a wash. |
| 8/20 | SEA @ MIL (Kirby/Gasser) | NRFI | 58% | _model-only_ | **L** (1st 0-6 → YRFI) | Kirby 3.89 / 1.31 with 7.51 K/9, last three at 3.24. Gasser 4.54 / 1.24 with 8.35 K/9 last three at 3.94. Dome, no weather input. ⚠ MIL scored 22 runs in this park two days ago — noted, not weighted. |
| 8/20 | NYY @ BAL (Cole/Bradish) | NRFI | 59% | _model-only_ | **W** (1st 0-0 → NRFI) | ⭐ **Cole's 1.08 WHIP over 86 IP is the best on the board** and last four (24.2 IP / 4 ER = 1.46 ERA) confirm it. Bradish 1.36 WHIP workable but last four 4.57. Camden is a hitter park; wind PENDING. |
| 8/20 | WSH @ TEX (Alvarez/deGrom) | YRFI | 57% | _model-only_ | **L** (1st 0-0 → NRFI) | ⚠ **deGrom's 3.95 season is stale-LOW — his last three are 3.1/5, 5.0/1, 2.0/2 = 7.13 ERA and one 2-IP outing** (short-leash 1st-inning red flag). Alvarez the opposite (2.59 recent). Globe Life dome, weather PENDING. |
| 8/20 | LAA @ HOU (G-Rod/Lambert) | YRFI | 60% | _model-only_ | **W** (1st 1-2 → YRFI) | ⭐ **Board's strongest lean. G-Rod 7.17 ERA / 1.59 WHIP is the worst 1st-inning quality on the board**; last three 4.80 still poor. Lambert 3.11 / 1.15 owns his half — read rests on LAA's top three vs a G-Rod strike-zone problem. Daikin roof, weather PENDING. |
| 8/22 | TOR @ NYY (Cease/Weathers) | NRFI | **64%** | _model-only_ | **L** (1st 1-0 → YRFI) | ⭐ Board's strongest lean either way. Cease 2.42/1.06/13.14 K/9 (L3 18.1 IP / 5 ER, 27 K) vs Weathers 3.56/1.19 with a L3 of **19.0 IP / 2 ER = 0.95**. 6.5 is the lowest full-game total on the slate by a full run, and the only live weather read on the board — **74°F, wind 6 mph IN from CF** — points the same way. |
| 8/22 | ATL @ MIL (M.Pérez/Henderson) | NRFI | 62% | _model-only_ | **L** (1st 1-0 → YRFI) | Two best WHIPs on the slate facing each other. **Henderson 2.70 / 0.87** (L3 19.1 IP / 6 ER, 21 K) vs **Martín Pérez 3.15 / 1.19** (L3 14.0 IP / 4 ER = 2.57). Retractable roof removes weather from the read. |
| 8/22 | MIN @ SD (Kremer/Mize) | NRFI | 55% | _model-only_ | **L** (1st 1-0 → YRFI) | Petco plus two arms outperforming their season lines. **Mize 3.18 / 1.07**; **Kremer's L3 17.1 IP / 6 ER = 3.12** against a 5.40 aggregate — E2 stale-HIGH on the Minnesota half. Held to 55 because the aggregate is genuinely bad. |
| 8/22 | TB @ BAL (McClanahan/B.Young) | YRFI | 54% | _model-only_ | **L** (1st 0-0 → NRFI) | ⚠ **C3 is the whole read** — McClanahan's last two outings are 3.0 and 3.1 IP after an 18-day gap. **Young's L3 14.1 IP / 8 ER = 5.02** is the worse half. Thin lean, logged for calibration. |
| 8/22 | DET @ KC (D.Anderson/Wacha) | YRFI | 54% | _model-only_ | **L** (1st 0-0 → NRFI) | ⚠ **E5 — Drew Anderson is a reliever mid-stretch-out** (5 GS across 76.1 IP; 3.2, 4.0, 5.0 IP most recently). A converted arm's 1st inning is his least-tested. **Wacha 3.58 / 1.15** holds the number down. Thinnest lean on the board. |
| 8/22 | ATH @ HOU (J.Lopez/H.Brown) | YRFI | 55% | _model-only_ | **L** (1st 0-0 → NRFI) | Houston's half is the read and it inverts the full-game market: **Hunter Brown's L3 is 16.0 IP / 8 ER = 4.50 with five walks in his most recent start** — walks are the most reliable 1st-inning YRFI driver. Lopez's 5.22/1.53 supports; his 3.00 L3 argues against. |
| 8/22 | PIT @ LAD (J.Jones/Skubal) | YRFI | 55% | _model-only_ | **W** (1st 3-0 → YRFI) | Jared Jones owns it: 4.78 / 1.20 understates an **L3 of 12.0 IP / 12 ER = 9.00** (incl. 3.0 IP / 8 ER on 8/9). **Skubal 2.85 / 0.96 is the best NRFI half on the board**, which is why this sits at 55 not 60. |
| 8/22 | WSH @ MIA (Irvin/E.Pérez) | YRFI | 56% | _model-only_ | **W** (1st 0-2 → YRFI) | **Irvin 5.80 / 1.36, L3 14.2 IP / 11 ER = 6.75.** Eury Pérez (3.23/1.11, L3 2.00) is a strong NRFI half, so this is one-sided — the same asymmetry the main build priced into a Miami ML, and the 8/20 lesson says those are different objects. |
| 8/22 | NYM @ CWS (C.Scott/Castillo) | YRFI | 57% | _model-only_ | **W** (1st 1-0 → YRFI) | Both halves worse than they look. **Castillo 4.99 / 1.37**; **Scott's 3.51 is stale-LOW — L3 15.1 IP / 10 ER = 5.87** incl. 4.0 IP / 4 ER on 8/10. |
| 8/22 | CIN @ AZ (Lowder/Soroka) | YRFI | 57% | _model-only_ | **W** (1st 1-0 → YRFI) | **Lowder 5.17 / 1.49** — second-worst WHIP among named starters, and WHIP is the 1st-inning metric. ⚠ Soroka is two starts off a two-month absence; a ramping arm's command is least settled early, so his 2.93 suppresses less than it implies. |
| 8/22 | LAA @ TEX (R.Johnson/Bradford) | YRFI | 58% | _model-only_ | **W** (1st 1-0 → YRFI) | **Ryan Johnson 6.41 / 1.54** is the worst combined line with real volume. ⚠ **Bradford's 2.93 rests on 15.1 IP / 3 GS with a 1.43 WHIP** — the ERA is not evidence, the WHIP is, and it agrees. |
| 8/22 | STL @ PHI (Q.Mathews/Painter) | YRFI | **60%** | _model-only_ | **W** (1st 0-3 → YRFI) | ⬆ **16:00 UPDATE — E4 LIFTED.** `recheck.py`: *AWAY probable now posted: Quinn Mathews (was TBA)*. The read **strengthens rather than flips**: Mathews' **1.56 WHIP** joins Painter's **1.55**, giving this game the **two worst WHIPs among named starters on the slate**, and WHIP is the 1st-inning metric. ⚠ Mathews' line rests on **2 GS / 9.0 IP**, so it is thin evidence in both directions — held to 60, not pushed higher. **58% → 60%.** |
| 8/22 | CHC @ SEA (Peterson/K.Anderson) | YRFI | 58% | _model-only_ | **W** (1st 0-1 → YRFI) | ⚠ **E4-class: Kade Anderson returns an EMPTY 2026 StatsAPI line — an MLB debut.** Highest-variance 1st inning on any board; **Peterson 5.29 / 1.53** supplies the other half. **Do not price at 16:00 without confirming the debut.** |
| 8/22 | SF @ BOS (Tidwell/Sandoval) | YRFI | 62% | _model-only_ | **W** (1st 1-0 → YRFI) | WHIP read end to end. **Sandoval's 1.85 is the worst on the slate by 0.31**, L3 15.0 IP / 11 ER = 6.60 incl. 4.0 IP / 7 ER on 8/16. Tidwell (1.22 over 3 GS) is thin evidence, not a suppressor. |
| 8/22 | CLE @ COL (Bibee/G.Hughes) | YRFI | **65% model / 58.2% derived** | **Over 0.5 −140 (FanDuel)** — no-vig **56.7%**, edge **+1.5pp** → ⛔ **NO BET (under the +2pp gate)** | **L** (1st 0-0 → NRFI) | ⭐ Board's strongest YRFI: Coors, the slate's highest total by 2.5 runs, and **Gabriel Hughes' L3 = 13.1 IP / 19 ER = 12.83**. Same construction that cashed here yesterday. Bibee (4.01/1.15) is the better half at a 5.50 L3. ⬆ **16:00 UPDATE — PRICED (1 credit) AND NOT PROMOTED; this row is the 8/12 audit in miniature.** Best pair **Over 0.5 −140 @FanDuel / Under 0.5 +125 @BetOnline.ag → no-vig 56.7% YRFI**. ⚠ **The 65% was a hand-model number, which doctrine treats as the `*`-equivalent of a TrueP.** Re-anchored to the market baseline, the only registered tag that applies is **`hitter_park_over+1.5`** (HALVED 8/19 from +3) — and **`wind_out_over` is RETIRED at 0.0pp**, despite Coors showing **10 mph out to RF**, which is precisely the trap that retirement exists to close. **TrueP 58.2%, edge +1.5pp — under the gate. NO BET.** The 6.8pp collapse is not a re-analysis; it is what happens when a number is required to name its adjustments. |
| 8/21 | ATL @ MIL (Sale/Misiorowski) | NRFI | 63% | _model-only_ | **W** (1st 0-0 → NRFI) | ⭐ **Board's strongest NRFI lean, and it is not close: the two best WHIPs on the slate face each other. |
| 8/21 | STL @ PHI (**Dobbins**/Luzardo) | NRFI | **55%** | _model-only_ | **L** (1st 2-0 → YRFI) | ⚙ **16:00 — E4 RESOLVED, read moves 54% → 55%.** St. Louis named **Hunter Dobbins (3.42 ERA / 1.30 WHIP / 8.82 K/9 over 50.0 IP; last three 16.1 IP / 5 ER = 2.76)** — solid recent form, which is why this goes to 55 and not to 60. ⚠ **His 1.30 WHIP is the half that caps it**, and the 8/20 lesson in this file is exactly that a middling WHIP concedes early traffic regardless of the full-game line. Philadelphia's half was never in doubt: **Luzardo 3.23 / 1.15 / 11.08 K/9, last three 21.2 IP / 3 ER = 1.26 with 28 K.** Citizens Bank is a genuine hitter park pulling the other way. **No `totals_1st_1_innings` market priced for this game → model-only, no bet.** |
| 8/21 | TOR @ NYY (Fluharty/Schlittler) | NRFI | 54% | _model-only_ | **W** (1st 0-0 → NRFI) | ⚠ **E5 — Toronto's listed probable Mason Fluharty is a RELIEVER**: 3 GS across 51. |
| 8/21 | SF @ BOS (Webb/Gray) | NRFI | 58% | **NRFI −155 (BetRivers) · no-vig 58.8%** | ⛔ **NO BET (−0.8pp)** | ⛔ **16:00 — PRICED AND DECLINED, which is the correct output rather than a failure.** This was the board's best *still-bettable* lean, so it was taken to a real market: NRFI −155 (BetRivers) / YRFI +135 (BetOnline.ag), hold 3.4% → **no-vig 58.8%** against a pre-registered **58%** = **edge −0.8pp**. The market is priced AHEAD of the model; betting it would mean paying the book to agree with us. ⚠ **The board's genuinely strongest lean — ATL @ MIL at 63% — was unbettable entirely: it went `In Progress` at 20:10Z, so the status gate closed on the one read worth chasing. A real timing cost of the 11:00-model / 16:00-price split.** **Two arms in the best form on the board. |
| 8/21 | WSH @ MIA (Lord/Gusto) | NRFI | 53% | _model-only_ | **W** (1st 0-0 → NRFI) | ⚠ **E5 — Washington's listed probable Brad Lord is a swingman**: 3 GS across 62. |
| 8/21 | TB @ BAL (F.Peralta/T.Rogers) | YRFI | 60% | _model-only_ | **W** (1st 0-1 → YRFI) | ⭐ **Strongest YRFI lean that is not Coors. |
| 8/21 | NYM @ CWS (Manaea/Burke) | YRFI | 55% | _model-only_ | **L** (1st 0-0 → NRFI) | **Burke's 3. |
| 8/21 | DET @ KC (Melton/Cameron) | NRFI | 62% | _model-only_ | **W** (1st 0-0 → NRFI) | ⭐ **Second-strongest NRFI, and the cleanest recent-form pair on the board. |
| 8/21 | ATH @ HOU (Ginn/Wesneski) | YRFI | 55% | _model-only_ | **L** (1st 0-0 → NRFI) | **Houston's half is the read. |
| 8/21 | LAA @ TEX (Detmers/Gore) | NRFI | 59% | _model-only_ | **W** (1st 0-0 → NRFI) | **Both season lines are stale-HIGH and both arms are hot — the rare case where E2 fires in the same direction on both halves. |
| 8/21 | CLE @ COL (Cantillo/Gordon) | YRFI | 64% | _model-only_ | **W** (1st 0-1 → YRFI) | ⭐ **Board's strongest lean in either direction: Coors Field plus the two worst WHIPs on the slate. |
| 8/21 | CIN @ AZ (Lodolo/E.Rodriguez) | YRFI | 56% | _model-only_ | **W** (1st 0-1 → YRFI) | ⚠ **E2 fires against Arizona: Eduardo Rodriguez's 2. |
| 8/21 | MIN @ SD (Prielipp/TBD) | YRFI | 55% | _model-only_ | **W** (1st 0-2 → YRFI) | ⚠ **E4 — San Diego have not named a probable, so half this read is unresolved. |
| 8/21 | PIT @ LAD (Chandler/Yamamoto) | NRFI | 56% | _model-only_ | **L** (1st 1-2 → YRFI) | **Yamamoto owns his half — 2. |
| 8/21 | CHC @ SEA (Boyd/Hancock) | YRFI | 54% | _model-only_ | **W** (1st 3-0 → YRFI) | **Thinnest lean on the board, logged for calibration rather than conviction. |

## Running totals (update on every settle)
- **Open (updated 8/25 11:00 Build A):** **7 of 15 reads for 8/25 are logged model-only** — 3 E4-gated (BOS@MIA, COL@WSH, BAL@STL); 8 games not yet modelled (wait for opposing SP confirmations). **8/24 CLOSED 4-6 (40%) — worst day since 8/9.** Strongest lean: **KC@TOR YRFI 62%** (Lugo/Scherzer both terrible). No reads priced until 16:00 `totals_1st_1_innings` sweep. _(prior 8/24 line follows)_ **Open (updated 8/24 11:00 Build A):** **10 of 10 reads for 8/24 are TBD** — full slate logged model-only; **8/23 CLOSED 8-7 and nothing earlier remains open.** ⚠ CIN @ SF is E4 (SF probable TBD) → one-sided read, LOW CONFIDENCE. Strongest lean: **PHI @ SEA NRFI 63%** (Wheeler/Gilbert at T-Mobile). No read priced until the 16:00 sweep. _(prior 8/23 line follows)_ **Open (updated 8/23 11:00 Build A):** **15 of 15 reads for 8/23 are TBD** — today's full slate is logged model-only; **8/22 is CLOSED at 7-6 and nothing earlier remains open.** ⚠ **Six of the fifteen are explicitly LOW CONFIDENCE because a bullpen game supplies one or both halves of the first inning** (DET @ KC on *both* sides, plus NYM @ CWS, SF @ BOS, ATH @ HOU and PIT @ LAD on one side each) — five bullpen games on one board is the most this tracker has recorded, and a starter-quality first-inning model has nothing to model in those half-innings. **Strongest leans: CHC @ SEA NRFI 62% and CLE @ COL YRFI 63%**, both built on two real starters. No read is priced until the 16:00 `totals_1st_1_innings` sweep.
- **Record:** **279-235 (54%)** (NRFI 135-113 · YRFI 144-122). _(**Updated 8/25 11:00 Build A** — 8/24 settled 4W-6L (40%): NRFI 2-3 (PIT@SD ✅, BOS@MIA ✅; PHI@SEA ❌, TB@DET ❌, CIN@SF ❌), YRFI 2-3 (MIN@ATH ✅, TEX@CWS ✅; CHC@AZ ❌, COL@WSH ❌, CLE@LAA ❌). **8/24 closed 4-6 — worst single-day slate since 8/9** — and the board's strongest lean (PHI@SEA NRFI 63%) was one of the five losses. Record moves 275-229 → 279-235. Prior line follows.)_ _(**Updated 8/24 11:00 Build A** — `nrfi_settle.py --apply` stamped all 15 8/23 rows off `linescore.innings[0]`; verified. **8/23 closed 8-7 (53%)** — NRFI leans 4-3, YRFI leans 4-4. The board's strongest lean (CHC@SEA NRFI 62%) lost as SEA scored early and CHC hung 19. Prior line follows.)_ _(prior: **249-211 (54%)** (NRFI 125-102 · YRFI 124-109). _(**Updated 8/21 11:00 Build A** — `nrfi_settle.py --apply` stamped **all 9 open 8/20 rows** off `linescore.innings[0]` at session start; stamps verified before this line was rewritten. **8/20 closed 6W-3L (67%)** — NRFI leans **4-1**, YRFI leans **2-2**. ⚠ **The day's most instructive row is LAA @ HOU**: the tracker's strongest lean (YRFI 60%, built on Grayson Rodriguez's 7.17 ERA / 1.59 WHIP) **won**, while the main build's `~own_sp_hi+5` mirror onto Houston — built on **the same line of the same pitcher** — lost by fifteen runs after G-Rod threw seven shutout innings. **A high WHIP predicts early traffic; it does not predict nine innings.** Recorded as a method note, not promoted. Today's 15 reads are raw model with no correction applied. Prior line follows.)_ _(prior: **243-208 (54%)** (NRFI 122-100 · YRFI 121-108). _(**Updated 8/20 11:00 Build A** — `nrfi_settle.py --apply` stamped **10 of the open 8/19 rows** off `linescore.innings[0]` at session start (LAD @ COL and one late row auto-settled; stamps verified). **8/19 closed 5W-10L (33%) — the tracker's weakest slate since 8/14.** ⚠ The story is that the NRFI side went **2-6** and the YRFI side went **3-4** — the third slate in five where a strongly-priced YRFI over Coors or an equally strong NRFI behind an ace failed. Recorded as directional at n=15 rather than promoted. ⚠ Today's 9 reads are the raw model with no correction applied (the −5pp YRFI shrink was RETIRED 8/19). Prior line follows.)_ _(prior: **240-197 (55%)** (NRFI 120-96 · YRFI 120-101). _(**Updated 8/19 11:00 Build A** — `nrfi_settle.py --apply` stamped **all 15 rows** of the 8/18 slate off `linescore.innings[0]` at session start; stamps verified before this line was rewritten. **8/18 closed 8-7 (53%)** — and the split is the story: **YRFI leans 7-3, NRFI leans 1-4**, the inverse of the pattern that had carried five of the previous six slates. ⛔ **The −5pp YRFI correction is RETIRED this run at its completed pre-registered review** (twelve graded slates against a ~10-slate registration; neutral overall, and a one-way ratchet toward NRFI on the two slates YRFI carried decisively). Today's reads are the raw model. ⚠ **Recorded, not promoted:** one 7-3/1-4 slate is n=15 and says nothing at the N≥20-30 bar — it is logged because it directly contradicts the directional note this file has been repeating since 8/13, and a pattern that reverses on its first real test was never a pattern. Prior line follows.)_ _(prior: **232-190 (55%)** (NRFI 119-92 · YRFI 113-98). _(**Updated 8/18 11:00 Build A** — `nrfi_settle.py --apply` stamped **all 11 rows** of the 8/17 slate off `linescore.innings[0]` at session start, and the stamps were **verified present** before this line was rewritten. ⭐ **8/17 CLOSED AT 9-2 (82%) — the tracker's best single slate on record.** NRFI leans **2-0**, YRFI leans **7-2**. **The two losses are both diagnosable rather than random:** AZ @ BOS (YRFI 57%) was flagged **E4** at publication with Boston's probable unlisted — half the read was admittedly unresolved — and LAD @ COL (YRFI 64%) was the board's strongest lean and the **first tracker read ever carried to a real price**, where the market's no-vig said 55.1%, we anchored to it, wrote **TrueP 56.6%**, called it under the +2pp gate and **NO BET** — and the first inning went **0-0**. ⭐ **The market-anchoring rule beat the tracker's own bare model number on its first live test, which is the single most useful calibration fact this file has produced.** ⚠ **The doubleheader bug logged 8/17 did NOT recur: today's slate has no doubleheader, so `nrfi_settle.py` resolved all eleven rows unattended — the bug remains open and untested rather than fixed.** Prior line follows.)_ _(prior: _(**Updated 8/17 18:00 Build C** — **STL @ CIN G1 settled BY HAND: 1st inning 2-0 → YRFI, tracker lean YRFI 62% → ✅ W.** ⚠ **It had to be settled by hand because `nrfi_settle.py` CANNOT RESOLVE A DOUBLEHEADER**: run at session start, at 16:00 and again at 18:00 it reported `STL @ CIN G1 … game not final (state=Pre-Game)` **while G1 was Final**, having matched the G1 row against the **G2** game object (same team pair, same date). ⚠ **This is the same collision class `settle.py` was hardened against on 7/29 — `nrfi_settle.py` never received that fix.** The failure mode is a **SILENT HOLE, not a wrong stamp** (the row stays TBD rather than settling wrong), which is exactly why it survived three `--apply` runs in one day unnoticed. **Proposed hardening: match on `gameNumber`.** The other ten 8/17 reads remain TBD — Pre-Game or In Progress at the 18:00 lock — and settle at tomorrow's 11:00 run.)_ _(superseded 8/17 11:00 line follows)_ **223-188 (54%)**. **NRFI:** **117-92 (56%)** · **YRFI:** **106-96 (52%)**. _(**Updated 8/17 11:00 Build A** — **8/16 is now FULLY SETTLED**; the six rows still open at the 18:00 lock were auto-stamped off `linescore.innings[0]` by `nrfi_settle.py --apply` at session start. **The 15-game slate closed 8W-7L (53%)**, and the split that Build C flagged mid-day held to the end: **the NRFI leans went 5-1 while the YRFI leans went 3-6.** ⚠ **That is the fifth slate in six carried by the NRFI side, and it is now the tracker's most persistent directional pattern** — recorded as directional, NOT promoted, since the per-side buckets across those six slates sit near n≈45 with the pattern resting on a handful of slates rather than a stable rate. ⚠ **The day's worst single read is a method lesson rather than a bad beat: KC @ LAA was leaned YRFI at 59% almost entirely on Ryan Johnson's 6.71 ERA, and the game produced a clean first inning inside a 3-run final.** A bad season ERA is a weak proxy for first-inning run risk; **today's 8/17 reads deliberately lean on WHIP and top-of-order quality wherever the two disagree with ERA.**)_ _(superseded 8/16 18:00 line follows)_ **219-186 (54%)**. **NRFI:** **115-91 (56%)** · **YRFI:** **104-95 (52%)**. _(**Updated 8/16 18:00 Build C** — nine of today's fifteen reads are decided and the day stands **4W-5L (44%)**, with **six still open** (BOS@PIT, SD@CLE, COL@SF, TEX@ATH, MIL@LAD in progress; SEA@HOU 19:20 ET). Settled today: NYY@TOR NRFI ✅, CWS@DET NRFI ✅, PHI@MIN YRFI ✅, STL@CHC YRFI ✅; BAL@TB YRFI ❌, AZ@ATL YRFI ❌, WSH@NYM YRFI ❌, MIA@CIN YRFI ❌, KC@LAA YRFI ❌. ⚠ **The split is the story and it runs against the recent pattern: the YRFI leans are 2-5 while the NRFI leans are 2-0.** Five YRFI reads priced at 56-62% all came back scoreless first innings — including MIA@CIN at 61%, the day's most confident YRFI lean, in the same game whose Over 8.5 the main build bet and lost. ⚠ **Recorded, not promoted:** one slate at n=9 says nothing at the N≥20-30 bar, and the standing observation that the NRFI side has carried four of the last five slates now has a fifth data point pointing the same way. **SEA@HOU stays model-only NO BET** — our 52% NRFI read sits UNDER the market's 56.7% no-vig, so there is no edge on either side.)_ _(superseded 8/16 11:00 line follows)_ **215-181 (54%)** (tracker opened 6/10/26). **NRFI:** **113-91 (55%)** · **YRFI:** **102-90 (53%)**. _(**Updated 8/16 11:00 Build A** — **8/15 is now FULLY SETTLED**; 12 rows auto-stamped off `linescore.innings[0]` by `nrfi_settle.py --apply` at session start. ⚠ Worth separating: the NRFI side carried the day again — the fourth slate in five where that is true, which is a directional pattern rather than a single result. It is NOT promoted to doctrine: the split is n≈40 across those five slates, well under the N≥20-30-per-bucket bar.)_ _(superseded 8/15 line follows)_ **207-174 (54%)** (tracker opened 6/10/26). **NRFI:** **107-87 (55%)** · **YRFI:** **100-87 (53%)**. _(**Updated 8/15 11:00 Build A** — **8/14 is now FULLY SETTLED**; 13 rows auto-stamped off `linescore.innings[0]` by `nrfi_settle.py --apply` at session start. The 14-game slate went **5W-8L**, the tracker's weakest full slate since 8/9 — and it lands on the same board where the main build's two gate-clearing legs also lost, i.e. the day was broadly hostile to modelled reads rather than to one method. ⚠ Recorded plainly rather than averaged away: the overall win rate has now slipped from 55% to **54%** and the YRFI side has given back its edge to **53%**.)_ _(superseded 8/14 line: 202-165 (55%), NRFI 104-84 · YRFI 98-81)_ _(**Updated 8/14 11:00 Build A** — 8/13 FULLY SETTLED at 6W-3L (67%).)_ **NRFI:** **104-84 (55%)** · **YRFI:** **98-81 (55%)**. _(**Updated 8/14 11:00 Build A** — **8/13 is now FULLY SETTLED at 6W-3L (67%)**, the tracker's best slate since 8/8. The three late rows were auto-stamped off `linescore.innings[0]` by `nrfi_settle.py --apply` at session start: TEX@LAA NRFI **W**, MIL@LAD YRFI **L**, PHI@MIN YRFI **W**. Full day: NRFI leans **4-1**, YRFI leans **2-2** — the third straight slate the NRFI side carried the board.)_ _(**Updated 8/13 18:00 Build C** — six of today's nine reads are decided and the day stands **4W-2L**: CLE@DET NRFI ✅, PIT@MIA NRFI ✅, SEA@NYY NRFI ✅, BOS@TOR NRFI ❌, CIN@CWS YRFI ✅, CHC@WSH YRFI ❌. NRFI leans **3-1**, YRFI leans **1-1**. The three west-coast/late rows stay TBD for tomorrow's 11:00 settle.)_ _(**8/9 is now FULLY SETTLED — 15 of 15 reads decided, and the day finished 10W-10L (50%)**; the five rows still open at the 18:30 Build C lock were auto-stamped off `linescore.innings[0]` at session start — DET@SF **W**, TB@SEA **L**, LAD@AZ **L**, CLE@CWS **L**, HOU@SD **L**. 8/8's 11-3 remains the tracker's best slate.)_)_)_)_
- **Staked:** $0 · **P/L:** $0.00 (model leans only — no priced bets yet; tracking calibration).
- **Open (updated 8/19 16:00 Build B):** **13 of 15 reads remain open; the two early games settled 0-2** (DET @ PIT and SD @ NYM both produced first-inning runs against NRFI leans — a 0-for-2 start that is noise at n=2 but is logged rather than smoothed). ⬆ **The ATH @ KC row is SUPERSEDED and FLIPPED** — the 11:00 ⛔ E4-on-both-halves flag lifts now that **Springs (6.17) / Lugo (4.59, 7.47 last three)** are posted, and the read inverts from a 51% NRFI coin-flip to a **58% YRFI** lean on the worst combined starting matchup on the board. ⚠ **THE PRICED SWEEP RAN AND PROMOTED NOTHING, for a structural reason worth naming: all three games pulled land in the ⛔ SUSPENDED 55-59 band.** Best-of-board devigs were **SF @ CLE NRFI −136/+115 → no-vig 55.3%**, **STL @ CIN NRFI −118/+100 → 52.0%**, **TOR @ TB NRFI −130/+110 → 54.3%**, against model leans of ~56-58% behind Messick, Burns and Rasmussen. **Shaded to market by the governor, every one prices at 0.0pp.** ⚠ **The suspension of band 55-59 (2-9, 18%) therefore disables essentially the whole NRFI product**, because a good starter's first-inning price naturally lives in that band. Cost recorded so it can be judged if the band re-warms and these reads were right. ⚠ **Weather now posted at 13 of 15 parks** (SEA @ MIL and LAD @ COL still PENDING); **SF @ CLE and LAA @ HOU still rest half their read on a pitcher with zero 2026 MLB innings.**
- **Open (updated 8/19 11:00 Build A):** **15 of 15 reads for 8/19 are TBD** — today's full slate is logged model-only; **8/18 is CLOSED at 8-7 and nothing earlier remains open.** ⚠ **Three of the fifteen are explicitly LOW CONFIDENCE: ATH @ KC is E4 on BOTH halves** (neither probable posted at 11:00), and **SF @ CLE and LAA @ HOU each rest half their read on a pitcher with zero 2026 MLB innings** (Matt Wilkinson, Ethan Pecko). ⚠ **Weather is PENDING at eleven of fifteen parks**; PNC, Citi, Target and Wrigley posted and are noted in-row. ⚠ **`wind_out_over` was retired from the registry this run**, so the three wind-out parks contribute 0.0pp to their reads — the change lands on this file's park half exactly as it lands on the main build's. 
- **Open (updated 8/18 11:00 Build A):** **15 of 15 reads for 8/18 are TBD** — today's full slate is logged model-only; **8/17 is CLOSED at 9-2 and nothing earlier remains open.** ⚠ **Three of the fifteen are explicitly LOW CONFIDENCE rather than modelled complete: ATH @ KC and CWS @ CHC are both E4-gated** (the probable is unlisted on one side), and **MIA @ PHI rests half its read on Cade Gibson, who has 0 GS across 33.2 IP and is an opener, not a starter.** Yesterday the single E4-gated read on the board was one of the day's only two losses, so those three carry the same warning that one did. ⚠ **Weather is PENDING at all fifteen parks at 11:00**, so every park half below is dome/geometry only — the 16:00 sweep owns the wind re-read and the priced `totals_1st_1_innings` pull.
- **Superseded (8/17 18:00 Build C):** **1 of 11 settled (STL @ CIN G1 ✅ W), 10 still TBD** — nine Pre-Game and BAL @ TB In Progress at the 18:00 lock. ⚠ **LAD @ COL YRFI HOLDS at 64%** now that Coors weather posted (**6 mph Out To LF at 88°F** confirms rather than moves the read) — still **model-only / NO BET**, since the priced version came in at +1.5pp against the +2pp gate. ⚠ **The standing tool note is now a confirmed BUG rather than a limitation:** the previous line said `nrfi_settle.py` only stamps once the GAME is Final; today showed it will not stamp a doubleheader game **even then**. _(superseded 8/17 11:00 line follows)_
- **Open (updated 8/17 11:00 Build A):** **11 of 11 reads TBD** — today's full slate is logged model-only; **8/16 is closed at 8-7 and nothing earlier remains open.** ⚠ **Two of the eleven are explicitly LOW CONFIDENCE rather than modelled as complete: AZ @ BOS is E4-gated** (Boston's probable is unlisted at 11:00) **and STL @ CIN G1 is an E5 bullpen game** (Kent Emanuel, **0 career GS**). ⚠ **Game 1 of the Cincinnati doubleheader first-pitches at 13:41 ET, before the 16:00 run** — it is model-only permanently, the same structural hole in the 11:00/16:00/18:00 cadence that 8/14 recorded for early games. ⚠ **Tool note, unchanged and still unfixed: `nrfi_settle.py` only stamps a row once the GAME is Final**, though the first inning decides these reads within ~20 minutes of first pitch — so the 18:00 run will again leave rows open that are already decided.
- **Superseded (8/14 11:00 Build A):** **14 of 14 reads TBD** — today's full slate is logged model-only; **8/13 is closed at 6-3 and nothing earlier remains open.** ⚠ **The −5pp YRFI correction enters slate NINE of its ~10-slate pre-registration.** Eight graded: 8/6 neutral, 8/7 neutral, 8/8 strongly positive, 8/9 strongly negative, 8/10 mildly negative, 8/11 neutral, 8/12 positive, 8/13 negative-on-the-flip. **Today it reverses exactly one read — MIL @ LAD**, raw YRFI 53% → NRFI 52%, flagged in-row. **Not tapered, not extended; the review is due after slate ten.** ⚠ **Three rows are E4-gated (TBA starters — BAL, ATL, ATH) and one is a bullpen game (CWS@DET, Newcomb 1 GS in 64.1 IP), so four of fourteen are explicitly LOW CONFIDENCE** rather than modelled as if complete. ⚠ **STL @ CHC first-pitches at 14:20 ET, before the 16:00 run** — its Wrigley wind input will never be re-read, which is a structural hole in the 11:00/16:00/18:00 cadence for early games rather than a miss on the day. ⚠ **Tool note, unchanged and still unfixed: `nrfi_settle.py` only stamps a row once the GAME is Final**, though the first inning decides these reads within ~20 minutes of first pitch — so the 18:00 run will again leave rows open that are already decided.
