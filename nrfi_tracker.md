...
- **Record:** **189-156** (tracker opened 6/10/26). **NRFI:** **95-81** · **YRFI:** **94-75**. _(**8/9 is now FULLY SETTLED — 15 of 15 reads decided, and the day finished 10W-10L (50%)**; the five rows still open at the 18:30 Build C lock were auto-stamped off `linescore.innings[0]` at session start — DET@SF **W**, TB@SEA **L**, LAD@AZ **L**, CLE@CWS **L**, HOU@SD **L**. 8/8's 11-3 remains the tracker's best slate.)_
- **Staked:** $0 · **P/L:** $0.00 (model leans only — no priced bets yet; tracking calibration).
- **Open:** **8/12 — 15 reads logged, all model-only; the two early games are settled and both WON.** ✅ **PHI @ STL NRFI 58% — 1st inning 0-0 → NRFI W** (the day's strongest read either way, off Wheeler's 0.98 WHIP, and it held even though Philadelphia went on to concede 7 later in the game). ✅ **BAL @ MIN YRFI 52% — 1st inning 1-1 → YRFI W** (Matthews at 5.23 was the softest qualified starter of the early window and both tops scored immediately; the 82°F / 5 mph in-from-centre counter did not bite). **Day 2-0; record now 189-156** (NRFI 95-81 · YRFI 94-75). **13 reads still open** (8 NRFI / 7 YRFI overall). ⚠ **Three data traps still standing in the open rows:** **HOU @ SF is a Houston bullpen game** (Bryan King, 0 GS / 49.1 IP — E5) and is deliberately flattened; **CHC @ WSH's Jackson Kent returns no 2026 StatsAPI pitching line at all**, so half that read is missing; **KC @ LAD's Daniel Lynch IV shows a 1.01 WHIP that is relief data** (1 GS in 47.2 IP) and is not used as a starter's number. Strongest open read is **TEX @ LAA YRFI 58%** off George Klassen's **2.88 WHIP over 8.2 IP**. ⚠ **Tool note: `nrfi_settle.py` only stamps a row once the GAME is Final**, even though the first inning decides these reads within ~20 minutes of first pitch — so a mid-slate settle leaves in-progress games open longer than the information requires. Recorded as a limitation, not fixed this run.
