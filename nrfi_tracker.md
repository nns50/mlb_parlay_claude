...
| 8/19 | DET @ PIT (Jobe/Skenes) | NRFI | 56% | _model-only_ | TBD | PNC is a pitcher park and **Skenes' 1.12 WHIP** owns the home half; the risk is all in Jobe's — 8.2 MLB IP all year at 1.38, and 6 ER in 3.2 IP on 8/14. Wind 9 mph OUT to LF at 77°F is a mild counter and, as of this run, contributes **0.0pp** — `wind_out_over` was retired from the registry today. |
| 8/19 | SD @ NYM (King/Stock) | YRFI | 60% | _model-only_ | TBD | ⭐ **Strongest soft-half on the board: Robert Stock's 1.86 WHIP** over 12.1 IP / 3 GS is the worst among today's 30 probables, and San Diego's top three are the better trio. Citi Field's suppression is the only thing holding this under 62%. |
...
| 8/19 | ATH @ KC (TBD/TBD) | NRFI | 51% | _model-only_ | TBD | ⛔ **E4 on BOTH halves — neither probable is posted at 11:00.** Written at a near-coin-flip with a slight Kauffman lean and **flagged as the least-informed read on the board**; the 16:00 run re-reads it once the names post. Do not treat 51% as a modelled number. |
